...
def get_price(ticker):
    ...
    change_pct = (
        ...
        if change != "N/A" and previous_close != "N/A"
        else "N/A"
    )
...
def create_pro_chart(ticker, period="6mo", chart_type="candlestick", indicators=None):
    ...
    if "rsi" in indicators and current_row < len(axes):
        ...
        ax_rsi.axhline(
            y=70, color="r", linestyle="--", alpha=0.7, label="Overbought (70)"
        )
        ax_rsi.axhline(
            y=30, color="g", linestyle="--", alpha=0.7, label="Oversold (30)"
        )
...
def generate_report(ticker, period="6mo"):
    ...
    if not hist.empty:
        ...
        forecast_range = current_price * (
            volatility / 100 / np.sqrt(52)
        )  # Weekly volatility

        print("\nNext Week Price Forecast (based on volatility):")
        print(
            f"Expected range: ${current_price - forecast_range:.2f} - ${current_price + forecast_range:.2f}"
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Stock Market Pro - Yahoo Finance Tool"
    )
    parser.add_argument(
        "command",
        choices=[
    ...
    parser.add_argument(
        "--line", action="store_true", help="Use line chart instead of candlestick"
    )
    ...
    try:
        if args.command == "price":
            ...
            print(
                f"https://unusualwhales.com/live-options-flow?ticker_symbol={args.ticker}"
            )
