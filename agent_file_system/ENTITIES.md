...
[c12c9261651a9] [pending] :: --- version: 7 purpose: agent operations manual ---
...
[c1c6d19d36638] [pending] :: For full step-by-step recipes per surface, follow these pointers. Do not duplicate them here. ``` Add an MCP server → ## MCP "Add or enable a server (recipe)...
[c1d8bd0c92d38] [pending] :: ...base.log, frontend_console.log └── .factory/ Factory machine state — do not touch ``` - `reference/requirements.md`: the contract. Any modify must append ...
...
[c22f951bbadd2] [pending] :: You can help the user connect external integrations directly through chat. Most token-based integrations can be fully driven by you: collect the credential f...
[c2315c2959e93] [pending] :: ... format and Atlassian email twitter: invalid signature API tier doesn't allow user upgrades Twitter API the operation tier (free is read-only) connection ...
...
[c232d69cd2c9c] [pending] :: Memory is your long-term recall. It is RAG-backed (relevance search over MEMORY.md and a few other files), not text-grep. Items reach MEMORY.md only after th...
...
[c28274da8ba13] [pending] :: The MemoryManager indexes a fixed set of files for semantic retrieval ([agent_core/core/impl/memory/manager.py](agent_core/core/impl/memory/manager.py), cons...
...
[c350bd335b901] [pending] :: <!-- index --> ``` how sessions/runs work → ## Runtime work a run / todos → ## Runs add MCP server → ## MCP add skill → ## Skills connect platform → ## Integ...
...
[c4f019d22e70f] [pending] :: ...ask, recurring_add, recurring_read, recurring_update_task, recurring_remove living_ui living_ui_scaffold, living_ui_list_projects, living_ui_notify_ready,...
...
[c52ce453686e3] [pending] :: From [MODEL_REGISTRY](agent_core/core/models/model_registry.py) — 13 providers:
...
[c54bd1e0a7a51] [pending] :: At construction (and on `reinitialize_llm`), `ModelFactory.create(provider, interface, model_override, ...)`: ``` 1. Looks up the provider in MODEL_REGISTRY[...
...
[c5770847a3107] [pending] :: ... find the section, OR call the manager's `retrieve_full_content(chunk_id)` if exposed via an action. Ranking is a weighted hybrid: `0.65 * vector similari...
[c58aa29927abe] [pending] :: You run inside `AgentBase.react(trigger)` at [app/agent_base.py](app/agent_base.py). The unit of work is a **session** (main, chat, or living_ui). A **run** ...
...
[c59c3ec846213] [pending] :: The MemoryManager indexes these files only ([agent_core/core/impl/memory/manager.py](agent_core/core/impl/memory/manager.py) `INDEX_TARGET_FILES`): ``` AGENT...
...
[c6400b9e3e472] [pending] :: Triggers are durable rows in per-session queues ([app/triggers/store.py](app/triggers/store.py), [agent_core/core/impl/trigger/session_queue.py](agent_core/c...
...
[c6f061844a639] [pending] :: ...ames are shared across the Google family for the same underlying account. Rules that matter to you: - **Every action for these integrations takes an optio...
[c6f3a15e52858] [pending] :: Read/write a project's live data with the lui CLI via `run_shell` (absolute paths required): ``` node <craftbot_root>/living-ui/tools/src/cli.ts data <projec...
...
[c759c120eb5e1] [pending] :: ...s remain after a prune) item_word_limit: int (default 150; words per stored memory item) model: llm_provider: "openai" | "anthropic" | "gemini" | "byteplu...
...
[c8291ba0d3edd] [pending] :: Living UI projects live at `agent_file_system/workspace/living_ui/<project_name>_<hash>/`. Every project is a React frontend + a single PocketBase backend pr...
...
[c93c801e81e73] [pending] :: The provider names used in code and in `model.llm_provider` are not always identical to the `api_keys.<key>` names: ``` provider name settings.json api_keys ...
...
[c9b6082164e33] [pending] :: - ALWAYS tell the user what credentials you need and where to get them. Never paste a vague "give me your token". - ALWAYS confirm the credential format roug...
[c9b6e90701640] [pending] :: These are not strictly "self" files (they're for output design, not agent behavior), but the agent edits them under similar discipline. See `## Documents` an...
[c9bea3867faa6] [pending] :: - Hand-writing a scaffold instead of `living_ui_scaffold`. Manual scaffolds miss the kit, ports, and registration. - Editing `frontend/src/kit/` or system-ma...
...
[cc5d9ec6f1a7b] [pending] :: The fields each token integration needs (declared per integration in `craftos_integrations/integrations/<name>/`; `connect_integration` returns `needs_creden...
...
[cd5d07b6259fc] [pending] :: Some persistent state the agent interacts with lives outside this directory: ``` app/config/settings.json model, API keys, OAuth, cache (## Configs) app/conf...
...
[cda67c9d77379] [pending] :: ``` Type of file → Destination final document the user should keep → workspace/<filename> draft, sketch, intermediate state, scratch → workspace/sessions/{se...
...
[ce55a54d3a620] [pending] :: `agent_file_system/workspace/` is your sandbox for task output. Three subdirectories with distinct lifecycles: ``` agent_file_system/workspace/ ├── <files at...
[ce77c8acc73c9] [pending] :: ``` Type of change File Consent rule ────────────────────────────────────────────────────── ──────────────── ────────────────────────────── Operational rule ...
