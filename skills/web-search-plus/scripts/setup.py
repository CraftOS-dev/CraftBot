...
def print_header():
    ...
    print()
    print(
        color("This wizard will help you configure your search providers.", Colors.DIM)
    )
...
def ask_choice(prompt: str, options: list, default: str = None) -> str:
    ...
    while True:
        ...
        response = input(
            f"Enter number (1-{len(options)}){color(hint, Colors.DIM)}: "
        ).strip()
        ...
        print(
            color(
                f"  Please enter a number between 1 and {len(options)}", Colors.YELLOW
            )
        )
...
def ask_api_key(provider: str, signup_url: str) -> str:
    ...
    print()
    print(
        f"  {color(f'Get your {provider} API key:', Colors.DIM)} {color(signup_url, Colors.BLUE)}"
    )
...
def ask_searxng_instance(docs_url: str) -> str:
    ...
    print()
    print(
        f"  {color('SearXNG is self-hosted. You need your own instance.', Colors.DIM)}"
    )
    print(f"  {color('Setup guide:', Colors.DIM)} {color(docs_url, Colors.BLUE)}")
    print()
    print(f"  {color('Example URLs:', Colors.DIM)}")
    print("    • http://localhost:8080 (local Docker)")
    print("    • https://searx.your-domain.com (self-hosted)")
    print()

    while True:
        url = input("  Enter your SearXNG instance URL: ").strip()

        if not url:
            print(
                color("    ⚠️  No URL entered. SearXNG will be disabled.", Colors.YELLOW)
            )
            return None
        ...
        if not url.startswith(("http://", "https://")):
            print(
                color("    ⚠️  URL must start with http:// or https://", Colors.YELLOW)
            )
            continue
        ...
        try:
            ...
            if _hostname in _blocked:
                print(
                    color(
                        f"    ❌ Blocked: {_hostname} is a cloud metadata endpoint.",
                        Colors.RED,
                    )
                )
                continue
            if not os.environ.get("SEARXNG_ALLOW_PRIVATE", "").strip() == "1":
                _resolved = socket.getaddrinfo(
                    _hostname, _parsed.port or 80, proto=socket.IPPROTO_TCP
                )
                for _fam, _t, _p, _cn, _sa in _resolved:
                    ...
                    if (
                        ...
                        print(
                            color(
                                f"    ❌ Blocked: {_hostname} resolves to private IP {_ip}.",
                                Colors.RED,
                            )
                        )
                        print(
                            color(
                                "       Set SEARXNG_ALLOW_PRIVATE=1 if intentional.",
                                Colors.DIM,
                            )
                        )
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=10) as response:
                ...
                if "results" in result or "query" in result:
                    print(
                        color(
                            "    ✓ Connection successful! SearXNG instance is working.",
                            Colors.GREEN,
                        )
                    )
                    return url.rstrip("/")
                    ...
                    print(
                        color(
                            "    ⚠️  Connected but response doesn't look like SearXNG JSON.",
                            Colors.YELLOW,
                        )
                    )
            ...
            if e.code == 403:
                print(
                    color("    ⚠️  JSON API is disabled (403 Forbidden).", Colors.YELLOW)
                )
                print(
                    color(
...
def ask_result_count() -> int:
    ...
    choice = ask_choice(
        "Default number of results per search?", options, "5 (balanced - recommended)"
    )
...
def run_setup(skill_dir: Path, force_reset: bool = False):
    ...
    if config_path.exists() and not force_reset:
        ...
        if not ask_yes_no("Do you want to reconfigure?", default=False):
            print(
                color("Setup cancelled. Your existing config is unchanged.", Colors.DIM)
            )
            return False
        print()
    ...
    providers_info = {
        ...
        "searxng": (
            ...
            "Privacy-first, self-hosted, $0 cost",
        ),
    }
    ...
    for provider, (name, url, desc) in providers_info.items():
        ...
        if provider == "searxng":
            print(
                color(
                    "    Note: SearXNG requires a self-hosted instance (no API key needed)",
                    Colors.DIM,
                )
            )
            if ask_yes_no("    Do you have a SearXNG instance?", default=False):
    ...
    if len(enabled_providers) > 1:
        ...
        print(
            f"Default provider: {color(default_provider, Colors.GREEN)} (only one enabled)"
        )
    ...
    print()
    print(
        color("Auto-routing", Colors.BOLD)
        + " automatically picks the best provider for each query:"
    )
    ...
    print(color('   python3 scripts/search.py -q "your query here"', Colors.CYAN))
    print()
