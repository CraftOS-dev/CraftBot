...
def _build_cache_payload(
    query: str, provider: str, max_results: int, params: Optional[Dict[str, Any]] = None
) -> Dict[str, Any]:
...
def cache_stats() -> Dict[str, Any]:
    ...
    entries = [
        p for p in CACHE_DIR.glob("*.json") if p.name != PROVIDER_HEALTH_FILE.name
    ]
    total_size = 0
    oldest_time = None
    newest_time = None
...
DEFAULT_CONFIG = {
    ...
    "serper": {"country": "us", "language": "en", "type": "search"},
    "tavily": {"depth": "basic", "topic": "general"},
    "exa": {"type": "neural"},
    "perplexity": {
        "api_url": "https://api.kilo.ai/api/gateway/chat/completions",
        "model": "perplexity/sonar-pro",
    },
    "you": {"country": "us", "safesearch": "moderate"},
    "searxng": {
...
def _validate_searxng_url(url: str) -> str:
    ...
    if parsed.scheme not in ("http", "https"):
        raise ValueError(
            f"SearXNG URL must use http or https scheme, got: {parsed.scheme}"
        )
    ...
    if hostname in BLOCKED_HOSTS:
        raise ValueError(
            f"SearXNG URL blocked: {hostname} is a cloud metadata endpoint"
        )
...
class QueryAnalyzer:
    ...
    def _calculate_signal_score(
        self, query: str, signals: Dict[str, float]
        ...
        for pattern, weight in signals.items():
            ...
            if found:
                ...
                matches.append(
                    {"pattern": pattern, "matched": match_text, "weight": weight}
                )
                total_score += weight
    ...
    def analyze(self, query: str) -> Dict[str, Any]:
        ...
        rag_score, rag_matches = self._calculate_signal_score(query, self.RAG_SIGNALS)
        ...
        if detected_url:
            ...
            discovery_matches.append(
                {"pattern": "url_detected", "matched": detected_url, "weight": 5.0}
            )
        ...
        provider_scores = {
            ...
            "tavily": research_score
            + (complexity["complexity_score"] if not complexity["is_complex"] else 0)
            + (0.2 * recency_score),
            "exa": discovery_score
            + (
                1.0
                if re.search(
                    r"\b(similar|alternatives?|examples?)\b", query, re.IGNORECASE
                )
                else 0.0
            ),
            "perplexity": direct_answer_score
            + (local_news_score * 0.4)
            + (recency_score * 0.55),
            "you": rag_score
            + (recency_score * 0.25),  # You.com good for real-time + RAG
    ...
    def route(self, query: str) -> Dict[str, Any]:
        ...
        available = {
            p: s for p, s in scores.items() if p not in disabled and get_env_key(p)
        }
        ...
        priority = self.auto_config.get(
            "provider_priority",
            ["tavily", "exa", "perplexity", "serper", "you", "searxng"],
        )
        ...
        if max_score == 0:
            ...
            second_best = (
                sorted(available.values(), reverse=True)[1] if len(available) > 1 else 0
            )
        ...
        return {
            ...
            "top_signals": [
                {"matched": s["matched"], "weight": s["weight"]} for s in top_signals
            ],
...
def explain_routing(query: str, config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "all_matches": {
            provider: [
                {"matched": m["matched"], "weight": m["weight"]} for m in matches
            ]
...
def mark_provider_failure(provider: str, error_message: str) -> Dict[str, Any]:
    ...
    cooldown_seconds = COOLDOWN_STEPS_SECONDS[
        min(fail_count - 1, len(COOLDOWN_STEPS_SECONDS) - 1)
    ]
    state[provider] = {
...
def deduplicate_results_across_providers(
    results_by_provider: List[Tuple[str, Dict[str, Any]]], max_results: int
...
def make_request(url: str, headers: dict, body: dict, timeout: int = 30) -> dict:
    ...
    try:
        ...
        raise ProviderRequestError(
            f"Request timed out after {timeout}s. Try again or reduce max_results.",
            transient=True,
        )
...
def search_you(
    ...
    try:
        ...
        raise ProviderRequestError(
            "You.com request timed out after 30s.", transient=True
        )
...
def search_searxng(
    ...
    try:
        ...
        is_timeout = "timed out" in reason.lower()
        raise ProviderRequestError(
            f"Cannot reach SearXNG instance at {instance_url}. Error: {reason}",
            transient=is_timeout,
        )
    except TimeoutError:
        raise ProviderRequestError(
            "SearXNG request timed out after 30s. Check instance health.",
            transient=True,
        )
    ...
    if data.get("answers"):
        answer = (
            data["answers"][0]
            if isinstance(data["answers"][0], str)
            else str(data["answers"][0])
        )
...
Full docs: See README.md and SKILL.md
    ...
    provider_priority = auto_config.get(
        "provider_priority", ["tavily", "exa", "perplexity", "serper"]
    )
    ...
    for p in providers_to_try:
        ...
        if in_cd:
            cooldown_skips.append(
                {"provider": p, "cooldown_remaining_seconds": remaining}
            )
        else:
    ...
    def execute_with_retry(prov: str) -> Dict[str, Any]:
        ...
        for attempt in range(0, 3):
            try:
                ...
                break
        raise (
            last_error if last_error else Exception("Unknown provider execution error")
        )
    ...
    if not args.no_cache and args.query:
        ...
        if cached_result:
            cache_hit = True
            result = {
                k: v for k, v in cached_result.items() if not k.startswith("_cache_")
            }
            result["cached"] = True
            result["cache_age_seconds"] = int(
                time.time() - cached_result.get("_cache_timestamp", 0)
            )
    ...
    if successful_results:
        if len(successful_results) == 1:
            ...
            deduped_results, dedup_count = deduplicate_results_across_providers(
                successful_results, args.max_results
            )
