---
name: agent-app-creator
description: Create Agent App applications (PocketBase backend, React kit frontend). Scaffolds, develops, validates, and launches local web apps with persistent state and realtime UI.
action-sets:
  - file_operations
  - code_execution
  - agent_app
...
| `AGENT_APP.md` | your plan/context/index — keep current |
...
0. **If `reference/requirements.md` starts with `MARKETPLACE DECISION: install
   ...
   `agent_app_marketplace_install(app_id=..., name=..., description=...,
...
1. Read `agent_file_system/GLOBAL_AGENT_APP.md` — colors, fonts, enforced rules.
2. Read `{project_path}/AGENT_APP.md` and `reference/requirements.md`. The
...
4. A Agent App build is substantial work — the standard run protocol applies
...
1. `agent_app_notify_ready(project_id="<PROJECT_ID>")` — runs the gate
...
2. **REALITY CHECK — look at what actually exists, not at what you wrote.**
   ...
   message reports.
3. `agent_app_walk_verify(project_id="<PROJECT_ID>")` — an independent
