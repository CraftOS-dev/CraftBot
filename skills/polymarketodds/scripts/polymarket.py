...
def format_market(market: dict, verbose: bool = False) -> str:
    ...
    if bid is not None and ask is not None:
        ...
        if spread > 0:
            lines.append(
                f"   Spread: {spread * 100:.1f}% (Bid: {format_price(bid)} / Ask: {format_price(ask)})"
            )
    ...
    if verbose:
        ...
        if week_change or month_change:
            lines.append(
                f"   📈 1w: {week_change or 'N/A'} | 1m: {month_change or 'N/A'}"
            )
...
def format_event(event: dict, show_all_markets: bool = False) -> str:
    ...
    if markets:
        ...
        display_count = (
            len(market_prices) if show_all_markets else min(10, len(market_prices))
        )
        for m, price in market_prices[:display_count]:
            ...
            if price > 0:
                lines.append(
                    f"   • {name}: {format_price(price)}{change_str} ({format_volume(vol)})"
                )
            else:
                lines.append(f"   • {name}")
...
def cmd_featured(args):
    ...
    params = {"closed": "false", "featured": "true", "limit": args.limit}
...
def cmd_watch(args):
    ...
    if args.action == "add":
        ...
        existing = [
            ...
            if w["slug"] == slug and w.get("outcome") == args.outcome
        ]
        if existing:
            watchlist["markets"] = [
                w
...
def cmd_calendar(args):
    ...
    data = fetch(
        "/events",
        {"closed": "false", "limit": 200, "order": "endDate", "ascending": "true"},
    )
    ...
    for dt, event in upcoming[: args.limit]:
        ...
        if markets:
            sorted_markets = sorted(
                markets, key=lambda m: get_market_price(m), reverse=True
            )
            if sorted_markets:
...
def cmd_movers(args):
    ...
    for m in movers[: args.limit]:
        ...
        print(
            f"   {change_pct:+.1f}% → Now {format_price(m['price'])} (Vol: {format_volume(m['volume'])})"
        )
        print()
...
def cmd_digest(args):
    ...
    data = fetch(
        "/events",
        {"closed": "false", "limit": 200, "order": "volume24hr", "ascending": "false"},
    )
    ...
    print(
        f"Markets: {len(matches)} | Volume: {format_volume(total_volume)} | 24h: {format_volume(total_24h)}"
    )
    print()
...
def cmd_portfolio(args):
    ...
    portfolio = load_json(
        "portfolio.json", {"positions": [], "history": [], "cash": 10000}
    )
    ...
    for pos in portfolio["positions"]:
        try:
            ...
            if data:
                ...
                print(
                    f"   {shares:.0f} shares @ {format_price(pos['entry_price'])} → {format_price(current_price)}"
                )
                print(
                    f"   Value: ${current_value:,.2f} | P&L: ${pnl:+,.2f} ({pnl_pct:+.1f}%)"
                )
                print()
        except Exception as e:
...
def cmd_buy(args):
    ...
    if amount > portfolio["cash"]:
        print(
            f"❌ Insufficient cash. Have: ${portfolio['cash']:,.2f}, Need: ${amount:,.2f}"
        )
        return
...
def cmd_sell(args):
    ...
    try:
        ...
        portfolio["positions"] = [
            p for p in portfolio["positions"] if p["slug"] != slug
        ]
        portfolio["history"].append(
            {
                "action": "sell",
...
def main():
    ...
    parser.add_argument(
        "--all", "-a", action="store_true", help="Show all markets in event"
    )
    ...
    search_parser.add_argument(
        "--all", "-a", action="store_true", help="Show all outcomes"
    )
    ...
    watch_parser.add_argument(
        "action", choices=["add", "remove", "list"], help="Action"
    )
    watch_parser.add_argument("slug", nargs="?", help="Event slug")
    watch_parser.add_argument("--outcome", "-o", help="Specific outcome to watch")
    watch_parser.add_argument(
        "--alert-at", type=float, help="Alert when price reaches X%"
    )
    watch_parser.add_argument(
        "--alert-change", type=float, help="Alert on X% change from entry"
    )

    # NEW: Alerts (for cron)
    alerts_parser = subparsers.add_parser("alerts", help="Check watchlist for alerts")
    alerts_parser.add_argument(
        "--quiet", "-q", action="store_true", help="Only output if alerts triggered"
    )
    ...
    movers_parser.add_argument(
        "--min-volume", type=float, default=10, help="Min 24h volume in $K"
    )
