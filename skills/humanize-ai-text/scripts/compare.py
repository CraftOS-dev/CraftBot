...
def main():
    ...
    parser.add_argument(
        "-a", "--aggressive", action="store_true", help="Use aggressive mode"
    )
    ...
    print(
        f"{'Issues':<25} {before.total_issues:<15} {after.total_issues:<15} {issue_sign}{issue_diff}"
    )
    ...
    print(
        f"{'Word Count':<25} {before.word_count:<15} {after.word_count:<15} {after.word_count - before.word_count:+}"
    )
