...
def show_report(symbol, ticker, info, period="6mo"):
    ...
    if _has_data(upper) and _has_data(lower) and (upper.iloc[-1] != lower.iloc[-1]):
        ...
        bb_label = (
            f" ({'Upper' if bb_pos > 80 else 'Lower' if bb_pos < 20 else 'Middle'})"
        )
...
def main():
    ...
    if cmd == "option":
        ...
        try:
            ...
            print(
                "[option] TIMEOUT: option scrape exceeded 25s and was safely aborted."
            )
            print(
                "[option] tip: free tier detailed flow is limited (JPM/INTC/IWM/XSP)."
            )
        return
