    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            async def get_val(label):
                try:
                    element = (
                        page.locator(f"text='{label}' >> xpath=..")
                        .locator("div,span,p")
                        .last
                    )
            ...
            sent_elem = (
                page.locator("div:has-text('🐂'), div:has-text('🐻')")
                .filter(has_text="%")
                .first
            )
            if await sent_elem.count() > 0:
                ...
                general_sentiment_match = await page.locator(
                    r"text=/\d+% (Bullish|Bearish|Neutral)/"
                ).first.inner_text()
            ...
            if ticker.upper() in FREE_FLOW_TICKERS:
                ...
                for row in data_rows[
                    :30
                ]:  # Limit to first 30 rows for efficiency and relevance
                    ...
                    if len(cols) >= 20:  # Ensure enough columns for parsing
                        try:
                            ...
                            prem_val = parse_val(
                                await cols[11].inner_text()
                            )  # Premium often col 11 or 12
                            vol_val = parse_val(
                                await cols[12].inner_text()
                            )  # Volume often col 12 or 13
                            oi_val = parse_val(
                                await cols[13].inner_text()
                            )  # OI often col 13 or 14
                            sent_val = await cols[
                                19
                            ].inner_text()  # Sentiment should be consistent
...
def print_advanced_report(ticker, data):
    ...
    if ov:
        ...
        sent_color = (
            "green"
            if "🐂" in sent_text or "Bullish" in sent_text
            else "red"
            if "🐻" in sent_text or "Bearish" in sent_text
            else "yellow"
        )
        ...
        console.print(
            Panel(
                ov_table,
                title=f"🐳 {ticker} Option Overview",
                border_style="bright_blue",
            )
        )
        console.print(
            Panel(
                f"Market Sentiment: [{sent_color}]{sent_text}[/{sent_color}]",
                box=None,
                justify="center",
            )
        )
        ...
        console.print(
            Panel(
                f"[red]Could not retrieve overview data for {ticker}.[/red]",
                border_style="red",
            )
        )
    ...
    if flow:
        ...
        insight_text += (
            f"📊 [bold]Recent Flow Sentiment:[/bold] {flow_sentiment_display}\n"
        )
        insight_text += (
            f"🐋 [bold]Large Whale Trades (>$100k):[/bold] {len(whales)} detected\n"
        )
        ...
        console.print(
            Panel(
                insight_text,
                title="🔍 Deep Insights from Live Flow",
                border_style="yellow",
            )
        )
        ...
        if whales:
            ...
            for w in whales[:5]:  # Show top 5 whale trades
                ...
                w_table.add_row(
                    f"${w['strike']}",
                    f"${w['premium']:,.0f}",
                    f"[{w_color}]{w['sentiment']}[/{w_color}]",
                )
            console.print(w_table)
    else:
        console.print(
            Panel(
                "[dim]Detailed live option flow data is not available for this ticker (free tier limitation or no recent trades).[/dim]",
                border_style="dim",
            )
        )
...
async def _run_with_timeout(ticker: str, total_timeout_s: int = 20):
    ...
    try:
        ...
        return await asyncio.wait_for(
            fetch_advanced_options(ticker), timeout=total_timeout_s
        )
...
if __name__ == "__main__":
    ...
    with console.status(
        f"[bold green]Scraping Option Data for {ticker} (UW, timeout {total_timeout_s}s)..."
    ):
