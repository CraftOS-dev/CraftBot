...
class TelegramBotSetup:
    ...
    def update_config(self, bot_token: str) -> bool:
        ...
        try:
            ...
            print("   - Pairing mode: True")
            print("   - Stream mode: partial")
            print("   - Plugin added: telegram")
            return True
    ...
    def show_next_steps(self, bot_token: str):
        ...
        print("3. The bot will provide pairing instructions")
        print("4. Follow the pairing process to link your Telegram account")

        print("\n🔧 Manual Commands:")
        print("   Check status: openclaw status")
        print("   View logs: openclaw gateway logs -f")
        print("   Restart: openclaw gateway restart")

        print("\n🧪 Testing:")
        print("   Run: python3 telegram-bot-manager/scripts/test_bot.py")
        print(f"   Or: export TELEGRAM_BOT_TOKEN={bot_token}")
        print("        python3 telegram-bot-manager/scripts/test_bot.py")

        print("\n📚 Documentation:")
        print("   - See references/OPENCLAW_CONFIG.md for detailed config")
        print("   - See references/WEBHOOK_SETUP.md for webhook setup")
