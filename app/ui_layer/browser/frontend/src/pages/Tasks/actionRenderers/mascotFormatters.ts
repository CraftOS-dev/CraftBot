...
import { basename, strField, arrField, dictField, boolField } from './parse'
...
const task_end: MascotActionFormatter = {
  // task_end is filtered out of narration upstream — the body reaction
  // (celebrate/frustrate animation) is the user-facing signal instead.
  // These fallbacks would only ever be reached by a routing bug; they
  // produce a plausible bubble rather than a crash.
  running: (i) => ({
    status: 'running',
    label: 'Finishing up',
    body: (strField(i, 'reason') ?? undefined) && firstSnippet(strField(i, 'reason')!, 60),
  }),
  result: (i, _o, s) => {
    const verb = s === 'completed' ? 'Task done' : s === 'error' ? 'Task failed' : 'Task cancelled'
    const summary = strField(i, 'summary') ?? ''
    return { status: s, label: verb, body: summary ? firstSnippet(summary, 60) : undefined }
  },
}

// TODOS ──────────────────────────────────────────────────────────────

const task_update_todos: MascotActionFormatter = {
  running: (i) => {
 ...
 *    "task_update_todos" → "Task update todos"
