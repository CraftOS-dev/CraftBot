/**
 * ConstructionDock — the read-only build visualizer shown while a Agent App is
 * being created.
 *
 ...
 * the app launches, AgentAppPage swaps this out for the running app iframe.
 */
...
import type { AgentAppProject, AgentAppBuildEvent } from '../../types'
import type { AgentAppTodo, AgentAppSnapshot } from '../../store/slices/agentAppSlice'
import styles from './AgentAppPage.module.css'

interface Props {
  project: AgentAppProject
  todos: AgentAppTodo[] | undefined
  events: AgentAppBuildEvent[]
  snapshot: AgentAppSnapshot | null
}
...
function deriveProgress(todos: AgentAppTodo[] | undefined): ProgressView {
  const list = todos ?? []
  if (list.length === 0) {
...
function usePacedEvents(events: AgentAppBuildEvent[]): AgentAppBuildEvent[] {
...
function coalesce(events: AgentAppBuildEvent[]): FeedRow[] {
  const rows: FeedRow[] = []
...
const AREA_ICONS: Record<AgentAppBuildEvent['area'], JSX.Element> = {
...
const KIND_ICONS: Partial<Record<AgentAppBuildEvent['kind'], JSX.Element>> = {
  read: <Eye size={13} />,
...
function eventIcon(e: AgentAppBuildEvent): JSX.Element {
  if (e.kind === 'test_run') {
...
export function ConstructionDock({ project, todos, events, snapshot }: Props) {
  ...
  const activityLabel =
    view.todoLabel ||
    (isLaunching
      ? t('agentapp:construction.finalChecks')
      : t('agentapp:construction.preparingWorkspace'))
      ...
      <div className={styles.dock}>
        <div className={styles.dockHeader}>
          <span className={styles.dockTitle}>
            {isLaunching ? (
              <>
                <Rocket size={13} /> {t('agentapp:construction.launching', { name: project.name })}
              </>
            ) : (
              <>{t('agentapp:construction.creating', { name: project.name })}</>
            )}
          </span>
        </div>
          ...
          <div className={styles.railChips}>
            {summary.components > 0 && (
              <PoppingChip value={summary.components}>
                <PanelsTopLeft size={11} /> {t('agentapp:construction.componentCount', { count: summary.components })}
              </PoppingChip>
            )}
            {summary.collections > 0 && (
              <PoppingChip value={summary.collections}>
                <Server size={11} /> {t('agentapp:construction.collectionCount', { count: summary.collections })}
              </PoppingChip>
            )}
            {summary.routes > 0 && (
              <PoppingChip value={summary.routes}>
                <Package size={11} /> {t('agentapp:construction.routeCount', { count: summary.routes })}
              </PoppingChip>
            )}
          </div>
        )}
