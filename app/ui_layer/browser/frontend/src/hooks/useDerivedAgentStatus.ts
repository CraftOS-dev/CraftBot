...
/**
 * Derives agent status from activity items and messages.
 *
...
export function useDerivedAgentStatus(
  ...
  return useMemo(() => {
    ...
    // Priority 2: something is running right now.
    const running = actions.filter(a => a.itemType === 'action' && a.status === 'running')
    if (running.length > 0) {
    ...
    // visibly acted since — it's still preparing a response.
