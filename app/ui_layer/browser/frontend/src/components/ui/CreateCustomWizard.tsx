...
import {
  ArrowLeft, ArrowRight, Box, ChevronDown, ChevronRight, Loader2, Paperclip,
  Sparkles, Upload, X,
} from 'lucide-react'
 ...
 *                 options, reference attachments
 *   2. INTERVIEW  LLM-generated questions (option chips + free text),
 *                 count scales with the app's complexity
 *   3. CREATING   requirements synthesis + project/task creation; on
 *                 success the caller navigates to the new project
 *
...
const OPTION_GROUPS: { key: string; label: string; choices: { value: string; label: string }[] }[] = [
  {
    ...
    label: 'Access',
    choices: [
      { value: 'Just me (no login)', label: 'Just me' },
      { value: 'Multiple users with login', label: 'Multi-user login' },
      { value: 'Team with roles (admin/member)', label: 'Team with roles' },
    ],
  },
  {
    key: 'style',
  ...
  },
  {
    key: 'fontSize',
    label: 'Font size',
    choices: ['Small', 'Medium', 'Large'].map(v => ({ value: v, label: v })),
  },
  {
    key: 'density',
    label: 'Element density',
    choices: ['Compact', 'Comfortable', 'Spacious'].map(v => ({ value: v, label: v })),
  },
  {
    key: 'corners',
    label: 'Corner style',
    choices: ['Sharp', 'Rounded', 'Pill'].map(v => ({ value: v, label: v })),
  },
  {
    key: 'motion',
    label: 'Motion',
...
export function CreateCustomWizard({ send, onMessage, onClose, onCreated }: CreateCustomWizardProps) {
  ...
  const setOption = (key: string, value: string) => {
    setOptions(prev => {
      const next = { ...prev }
      if (next[key] === value) delete next[key] // click again -> agent decides
      else next[key] = value
      return next
    })
  }
  ...
  const uploadStaged = async (): Promise<boolean> => {
    ...
    try {
      const uploadOne = async (file: File, kind: 'reference' | 'icon') => {
        ...
        const params = new URLSearchParams({ wizardId: wid, name: file.name, kind })
        const resp = await fetch(`/api/living-ui/wizard-upload?${params}`, {
          method: 'POST',
          body: form,
        })
        const result = await resp.json()
        if (!result.success) throw new Error(result.error || `Upload failed: ${file.name}`)
      }
        ...
        <div className={styles.advanced}>
          ...
          >
            ...
            <span className={styles.advancedHint}>
              style, density, access, motion — all optional
            </span>
          </button>
          ...
          {showAdvanced && (
            <div className={styles.advancedBody}>
              {OPTION_GROUPS.map(group => (
                <div key={group.key} className={styles.advancedRow}>
                  <label className={styles.advancedLabel}>{group.label}</label>
                  <div className={styles.advancedOptions}>
                    <button
                      type="button"
                      className={`${styles.chip} ${!options[group.key] ? styles.chipDefaultOn : ''}`}
                    ...
                    {group.choices.map(choice => (
                      <button
                        key={choice.value}
                        type="button"
                        className={`${styles.chip} ${options[group.key] === choice.value ? styles.chipActive : ''}`}
                        onClick={() => setOption(group.key, choice.value)}
                      >
                        {choice.label}
                      </button>
                    ))}
