...
/**
 * Curated icon set for Agent App projects. The project's `icon` field is
 * either "lucide:<Name>" (picked from this set) or "file:<relpath>"
 * (uploaded, served by GET /api/agent-app/icon/<projectId>). Anything else
 * (or nothing) falls back to the classic cube.
 */
export const AGENT_APP_ICONS: Record<string, LucideIcon> = {
...
export function AgentAppIcon({ icon, projectId, size = 13, className }: AgentAppIconProps) {
  if (icon?.startsWith('lucide:')) {
    const Cmp = AGENT_APP_ICONS[icon.slice(7)]
    if (Cmp) return <Cmp size={size} className={className} />
  }
  if (icon?.startsWith('file:')) {
    return (
      <img
        src={`/api/agent-app/icon/${projectId}`}
        alt=""
        width={size}
        height={size}
        className={className}
