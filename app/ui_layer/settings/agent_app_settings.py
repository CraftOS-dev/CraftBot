"""Agent App settings management for UI layer.

Provides functions for managing Agent App project settings
...
def get_agent_app_projects() -> Dict[str, Any]:
    ...
    try:
        ...
        for project in manager.list_projects():
            # Backup status (spec agent-app-backups-plan Phase 4): sidecar
...
def update_project_setting(project_id: str, setting: str, value: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        manager = get_agent_app_manager()
        if not manager:
            return {"success": False, "error": "Agent App manager not initialized"}
