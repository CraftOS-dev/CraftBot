...
@action(
    ...
    input_schema={
        "folder_id": {"type": "string", "description": "Google Drive folder ID. Use 'root' for the user's My Drive.", "example": "root"},
    },
...
@action(
    ...
    action_sets=["google_drive_files", "google_drive"],
    input_schema={
        "query": {"type": "string", "description": "Drive q-query.", "example": "name contains 'budget' and trashed = false"},
        "max_results": {"type": "integer", "description": "Max results.", "example": 50},
    },
    output_schema={"status": {"type": "string", "example": "success"}},
)
def search_drive_files(input_data: dict) -> dict:
    from app.data.action.integrations._helpers import run_client_sync
    return run_client_sync(
        "google_drive", "search_drive",
        unwrap_envelope=True, fail_message="Failed to search files.",
...
@action(
    ...
    input_schema={
        ...
        "fields": {"type": "string", "description": "Comma-separated field list (optional).", "example": ""},
    },
    output_schema={"status": {"type": "string", "example": "success"}},
)
def get_drive_file(input_data: dict) -> dict:
    from app.data.action.integrations._helpers import run_client_sync
    return run_client_sync(
        "google_drive", "get_drive_file",
        unwrap_envelope=True, fail_message="Failed to get file.",
...
@action(
    ...
    action_sets=["google_drive_files", "google_drive"],
    input_schema={
        "file_path": {"type": "string", "description": "Absolute path to the local file.", "example": "C:/Users/me/report.pdf"},
        "name": {"type": "string", "description": "Drive filename (defaults to local filename).", "example": ""},
        "mime_type": {"type": "string", "description": "MIME type (defaults to autodetect).", "example": ""},
        "parent_folder_id": {"type": "string", "description": "Target folder ID (defaults to root).", "example": ""},
    },
    output_schema={"status": {"type": "string", "example": "success"}},
    parallelizable=False,
)
def upload_drive_file(input_data: dict) -> dict:
    ...
    return run_client_sync(
        ...
        unwrap_envelope=True, fail_message="Failed to upload file.",
...
@action(
    ...
    input_schema={
        "file_id": {"type": "string", "description": "Drive file ID to overwrite.", "example": ""},
        "file_path": {"type": "string", "description": "Absolute path to the new local content.", "example": "C:/Users/me/report_v2.pdf"},
        "mime_type": {"type": "string", "description": "MIME type (defaults to autodetect).", "example": ""},
    },
    output_schema={"status": {"type": "string", "example": "success"}},
    parallelizable=False,
...
def update_drive_file_content(input_data: dict) -> dict:
    from app.data.action.integrations._helpers import run_client_sync
    return run_client_sync(
        "google_drive", "update_drive_file_content",
        unwrap_envelope=True, fail_message="Failed to update file content.",
        file_id=input_data["file_id"],
        file_path=input_data["file_path"],
        mime_type=input_data.get("mime_type") or None,
    )
...
@action(
    ...
    input_schema={
        "file_id": {"type": "string", "description": "File ID.", "example": ""},
        "save_to": {"type": "string", "description": "Local path to save to. Parent directories will be created.", "example": "C:/Users/me/downloads/report.pdf"},
    },
    output_schema={"status": {"type": "string", "example": "success"}},
    parallelizable=False,
)
def download_drive_file(input_data: dict) -> dict:
    from app.data.action.integrations._helpers import run_client_sync
    return run_client_sync(
        "google_drive", "download_drive_file",
        unwrap_envelope=True, fail_message="Failed to download file.",
        file_id=input_data["file_id"],
...
@action(
    ...
    action_sets=["google_drive_files", "google_drive"],
    input_schema={
        "file_id": {"type": "string", "description": "Google-native file ID.", "example": ""},
        "save_to": {"type": "string", "description": "Local path to save to.", "example": "C:/Users/me/report.pdf"},
        "mime_type": {"type": "string", "description": "Target export MIME type.", "example": "application/pdf"},
    },
    output_schema={"status": {"type": "string", "example": "success"}},
    parallelizable=False,
)
def export_drive_file(input_data: dict) -> dict:
    from app.data.action.integrations._helpers import run_client_sync
    return run_client_sync(
        "google_drive", "export_drive_file",
        unwrap_envelope=True, fail_message="Failed to export file.",
        file_id=input_data["file_id"],
        save_to=input_data["save_to"],
...
@action(
    ...
    input_schema={
        ...
        "name": {"type": "string", "description": "Name for the copy (optional).", "example": ""},
        "parent_folder_id": {"type": "string", "description": "Target folder ID (optional).", "example": ""},
    },
    output_schema={"status": {"type": "string", "example": "success"}},
    parallelizable=False,
)
def copy_drive_file(input_data: dict) -> dict:
    from app.data.action.integrations._helpers import run_client_sync
    return run_client_sync(
        "google_drive", "copy_drive_file",
        unwrap_envelope=True, fail_message="Failed to copy file.",
        file_id=input_data["file_id"],
        name=input_data.get("name") or None,
        parent_folder_id=input_data.get("parent_folder_id") or None,
    )
...
@action(
    ...
    input_schema={
        ...
        "starred": {"type": "boolean", "description": "Star/unstar (optional).", "example": False},
        "trashed": {"type": "boolean", "description": "Send to trash without deleting (optional).", "example": False},
    },
    output_schema={"status": {"type": "string", "example": "success"}},
    parallelizable=False,
)
def update_drive_file_metadata(input_data: dict) -> dict:
    from app.data.action.integrations._helpers import run_client_sync
    return run_client_sync(
        "google_drive", "update_drive_file_metadata",
        unwrap_envelope=True, fail_message="Failed to update file.",
        file_id=input_data["file_id"],
        name=input_data.get("name") or None,
...
def delete_drive_file(input_data: dict) -> dict:
    from app.data.action.integrations._helpers import run_client_sync
    return run_client_sync(
        "google_drive", "delete_drive_file",
        unwrap_envelope=True, fail_message="Failed to delete file.",
        file_id=input_data["file_id"],
    )
...
def empty_drive_trash(input_data: dict) -> dict:
    from app.data.action.integrations._helpers import run_client_sync
    return run_client_sync(
        "google_drive", "empty_drive_trash",
        unwrap_envelope=True, fail_message="Failed to empty trash.",
    )
...
def get_drive_about(input_data: dict) -> dict:
    from app.data.action.integrations._helpers import run_client_sync
    return run_client_sync(
        "google_drive", "get_drive_about",
        unwrap_envelope=True, fail_message="Failed to get Drive info.",
    )
...
@action(
    ...
    input_schema={
        "file_id": {"type": "string", "description": "File or folder ID.", "example": ""},
    },
    output_schema={"status": {"type": "string", "example": "success"}},
)
def list_drive_permissions(input_data: dict) -> dict:
    from app.data.action.integrations._helpers import run_client_sync
    return run_client_sync(
        "google_drive", "list_drive_permissions",
        unwrap_envelope=True, fail_message="Failed to list permissions.",
        file_id=input_data["file_id"],
    )
...
@action(
    ...
    input_schema={
        "file_id": {"type": "string", "description": "File ID.", "example": ""},
        "permission_id": {"type": "string", "description": "Permission ID.", "example": ""},
    },
    output_schema={"status": {"type": "string", "example": "success"}},
)
def get_drive_permission(input_data: dict) -> dict:
    from app.data.action.integrations._helpers import run_client_sync
    return run_client_sync(
        "google_drive", "get_drive_permission",
        unwrap_envelope=True, fail_message="Failed to get permission.",
        file_id=input_data["file_id"],
...
@action(
    ...
    input_schema={
        "file_id": {"type": "string", "description": "File or folder ID.", "example": ""},
        "role": {"type": "string", "description": "reader, commenter, writer, or owner.", "example": "reader"},
        "perm_type": {"type": "string", "description": "user, group, domain, or anyone.", "example": "user"},
        "email_address": {"type": "string", "description": "Email (for user/group types).", "example": "alice@example.com"},
        "domain": {"type": "string", "description": "Domain (for domain type).", "example": ""},
        "send_notification": {"type": "boolean", "description": "Email the grantee.", "example": True},
        "email_message": {"type": "string", "description": "Custom notification message (optional).", "example": ""},
    },
    output_schema={"status": {"type": "string", "example": "success"}},
    parallelizable=False,
)
def add_drive_permission(input_data: dict) -> dict:
    from app.data.action.integrations._helpers import run_client_sync
    return run_client_sync(
        "google_drive", "create_drive_permission",
        unwrap_envelope=True, fail_message="Failed to add permission.",
        file_id=input_data["file_id"],
...
def update_drive_permission(input_data: dict) -> dict:
    from app.data.action.integrations._helpers import run_client_sync
    return run_client_sync(
        "google_drive", "update_drive_permission",
        unwrap_envelope=True, fail_message="Failed to update permission.",
        file_id=input_data["file_id"],
        permission_id=input_data["permission_id"],
        role=input_data["role"],
    )
...
def remove_drive_permission(input_data: dict) -> dict:
    from app.data.action.integrations._helpers import run_client_sync
    return run_client_sync(
        "google_drive", "delete_drive_permission",
        unwrap_envelope=True, fail_message="Failed to remove permission.",
        file_id=input_data["file_id"],
        permission_id=input_data["permission_id"],
    )
...
@action(
    ...
    input_schema={
        "file_id": {"type": "string", "description": "File ID.", "example": ""},
        "include_deleted": {"type": "boolean", "description": "Include soft-deleted comments.", "example": False},
    },
    output_schema={"status": {"type": "string", "example": "success"}},
)
def list_drive_comments(input_data: dict) -> dict:
    from app.data.action.integrations._helpers import run_client_sync
    return run_client_sync(
        "google_drive", "list_drive_comments",
        unwrap_envelope=True, fail_message="Failed to list comments.",
        file_id=input_data["file_id"],
...
def get_drive_comment(input_data: dict) -> dict:
    from app.data.action.integrations._helpers import run_client_sync
    return run_client_sync(
        "google_drive", "get_drive_comment",
        unwrap_envelope=True, fail_message="Failed to get comment.",
        file_id=input_data["file_id"],
...
@action(
    ...
    input_schema={
        "file_id": {"type": "string", "description": "File ID.", "example": ""},
        "content": {"type": "string", "description": "Comment text.", "example": "Please review."},
        "anchor": {"type": "string", "description": "Optional anchor (structured format).", "example": ""},
    },
    output_schema={"status": {"type": "string", "example": "success"}},
    parallelizable=False,
)
def create_drive_comment(input_data: dict) -> dict:
    from app.data.action.integrations._helpers import run_client_sync
    return run_client_sync(
        "google_drive", "create_drive_comment",
        unwrap_envelope=True, fail_message="Failed to create comment.",
        file_id=input_data["file_id"],
...
@action(
    ...
    input_schema={
        "file_id": {"type": "string", "description": "File ID.", "example": ""},
        "comment_id": {"type": "string", "description": "Comment ID.", "example": ""},
        "content": {"type": "string", "description": "New content (optional).", "example": ""},
        "resolved": {"type": "boolean", "description": "Mark as resolved (optional).", "example": True},
    },
    output_schema={"status": {"type": "string", "example": "success"}},
    parallelizable=False,
)
def update_drive_comment(input_data: dict) -> dict:
    from app.data.action.integrations._helpers import run_client_sync
    return run_client_sync(
        "google_drive", "update_drive_comment",
        unwrap_envelope=True, fail_message="Failed to update comment.",
        file_id=input_data["file_id"],
        comment_id=input_data["comment_id"],
...
def delete_drive_comment(input_data: dict) -> dict:
    from app.data.action.integrations._helpers import run_client_sync
    return run_client_sync(
        "google_drive", "delete_drive_comment",
        unwrap_envelope=True, fail_message="Failed to delete comment.",
        file_id=input_data["file_id"],
        comment_id=input_data["comment_id"],
    )
...
def list_drive_comment_replies(input_data: dict) -> dict:
    from app.data.action.integrations._helpers import run_client_sync
    return run_client_sync(
        "google_drive", "list_drive_comment_replies",
        unwrap_envelope=True, fail_message="Failed to list replies.",
        file_id=input_data["file_id"],
        comment_id=input_data["comment_id"],
    )
...
def create_drive_comment_reply(input_data: dict) -> dict:
    from app.data.action.integrations._helpers import run_client_sync
    return run_client_sync(
        "google_drive", "create_drive_comment_reply",
        unwrap_envelope=True, fail_message="Failed to create reply.",
        file_id=input_data["file_id"],
        comment_id=input_data["comment_id"],
        content=input_data["content"],
    )
...
def update_drive_comment_reply(input_data: dict) -> dict:
    from app.data.action.integrations._helpers import run_client_sync
    return run_client_sync(
        "google_drive", "update_drive_comment_reply",
        unwrap_envelope=True, fail_message="Failed to update reply.",
        file_id=input_data["file_id"],
        comment_id=input_data["comment_id"],
...
def delete_drive_comment_reply(input_data: dict) -> dict:
    from app.data.action.integrations._helpers import run_client_sync
    return run_client_sync(
        "google_drive", "delete_drive_comment_reply",
        unwrap_envelope=True, fail_message="Failed to delete reply.",
        file_id=input_data["file_id"],
        comment_id=input_data["comment_id"],
        reply_id=input_data["reply_id"],
    )
...
def list_drive_revisions(input_data: dict) -> dict:
    from app.data.action.integrations._helpers import run_client_sync
    return run_client_sync(
        "google_drive", "list_drive_revisions",
        unwrap_envelope=True, fail_message="Failed to list revisions.",
        file_id=input_data["file_id"],
    )
...
def get_drive_revision(input_data: dict) -> dict:
    from app.data.action.integrations._helpers import run_client_sync
    return run_client_sync(
        "google_drive", "get_drive_revision",
        unwrap_envelope=True, fail_message="Failed to get revision.",
        file_id=input_data["file_id"],
...
@action(
    ...
    input_schema={
        "file_id": {"type": "string", "description": "File ID.", "example": ""},
        "revision_id": {"type": "string", "description": "Revision ID.", "example": ""},
        "keep_forever": {"type": "boolean", "description": "Pin this revision (otherwise Drive auto-prunes after 100 or 30 days, whichever first).", "example": True},
        "published": {"type": "boolean", "description": "Publish state (Google-native files only).", "example": False},
        "publish_auto": {"type": "boolean", "description": "Auto-publish subsequent revisions.", "example": False},
    },
    output_schema={"status": {"type": "string", "example": "success"}},
    parallelizable=False,
)
def update_drive_revision(input_data: dict) -> dict:
    from app.data.action.integrations._helpers import run_client_sync
    return run_client_sync(
        "google_drive", "update_drive_revision",
        unwrap_envelope=True, fail_message="Failed to update revision.",
        file_id=input_data["file_id"],
        revision_id=input_data["revision_id"],
        keep_forever=input_data["keep_forever"] if "keep_forever" in input_data else None,
        published=input_data["published"] if "published" in input_data else None,
        publish_auto=input_data["publish_auto"] if "publish_auto" in input_data else None,
    )
...
def delete_drive_revision(input_data: dict) -> dict:
    from app.data.action.integrations._helpers import run_client_sync
    return run_client_sync(
        "google_drive", "delete_drive_revision",
        unwrap_envelope=True, fail_message="Failed to delete revision.",
        file_id=input_data["file_id"],
        revision_id=input_data["revision_id"],
    )
...
@action(
    ...
    input_schema={
        ...
        "q": {"type": "string", "description": "Drive search query (optional).", "example": ""},
    },
    output_schema={"status": {"type": "string", "example": "success"}},
)
def list_shared_drives(input_data: dict) -> dict:
    ...
    return run_client_sync(
        ...
        unwrap_envelope=True, fail_message="Failed to list shared drives.",
...
@action(
    ...
    action_sets=["google_drive_shared_drives"],
    input_schema={
        "drive_id": {"type": "string", "description": "Shared drive ID.", "example": ""},
    },
    output_schema={"status": {"type": "string", "example": "success"}},
)
def get_shared_drive(input_data: dict) -> dict:
    ...
    return run_client_sync(
        ...
        unwrap_envelope=True, fail_message="Failed to get shared drive.",
...
@action(
    ...
    action_sets=["google_drive_shared_drives"],
    input_schema={
        "name": {"type": "string", "description": "Shared drive name.", "example": "Team project"},
    },
    output_schema={"status": {"type": "string", "example": "success"}},
    parallelizable=False,
)
def create_shared_drive(input_data: dict) -> dict:
    ...
    return run_client_sync(
        ...
        unwrap_envelope=True, fail_message="Failed to create shared drive.",
        name=input_data["name"],
    )
...
def update_shared_drive(input_data: dict) -> dict:
    ...
    return run_client_sync(
        ...
        unwrap_envelope=True, fail_message="Failed to update shared drive.",
        drive_id=input_data["drive_id"],
        name=input_data.get("name") or None,
...
def delete_shared_drive(input_data: dict) -> dict:
    ...
    return run_client_sync(
        ...
        unwrap_envelope=True, fail_message="Failed to delete shared drive.",
        drive_id=input_data["drive_id"],
    )
