...
def list_slack_channels(input_data: dict) -> dict:
    ...
    res = run_client_sync("slack", "list_channels", account=input_data.get("account"), limit=input_data.get("limit", 100))
    if input_data.get("include_metadata") or res.get("status") != "success":
        return res
    body = res.get("result")
    if not isinstance(body, dict):
...
def list_slack_users(input_data: dict) -> dict:
    ...
    res = run_client_sync("slack", "list_users", account=input_data.get("account"), limit=input_data.get("limit", 100))
    if input_data.get("include_metadata") or res.get("status") != "success":
        return res
    body = res.get("result")
    if not isinstance(body, dict):
