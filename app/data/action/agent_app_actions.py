"""Agent App actions for agent to notify UI status and progress."""
...
@action(
    name="agent_app_scaffold",
    description=(
        "Create and register a new Agent App project from the template, then "
        "dispatch the build to the project's dedicated session. Call this when "
        "the user asks for a new Agent App in a regular chat — i.e. when your "
        "task instruction does NOT already contain a 'Project ID' and 'Project "
        "Path' (those come pre-scaffolded from the Create Agent App modal). "
    ...
    input_schema={
        "name": {
            ...
            "description": "Display name for the Agent App project.",
        },
        "description": {
            "type": "string",
    ...
    output_schema={
        ...
        "project_path": {
            "type": "string",
            "example": "/workspace/agent_app/stock_forecaster_abc12345",
    ...
    test_payload={
        "name": "Test App",
        "description": "A test Agent App.",
        "simulated_mode": True,
    },
)
async def agent_app_scaffold(input_data: dict) -> dict:
    """Create, register, and associate a new Agent App project from the template."""
    ...
    if simulated_mode:
        return {
            "status": "success",
            "project_id": "abc12345",
            "project_path": "/workspace/agent_app/test_app_abc12345",
            "frontend_port": 3100,
            "backend_port": 3101,
    ...
    try:
        ...
        if not manager:
            return {
                "status": "error",
                "message": (
                    "Agent App manager not initialized. Agent App creation requires "
        ...
        await broadcast_agent_app_created(project.to_dict())
        ...
        # exists, marks the project as creating, and fires a AGENT_APP_DEV
        # trigger carrying the full build instruction, so todos/progress/
        # questions stream to the Agent App view.
        ...
        return {
            ...
            "message": (
                ...
                f'call agent_app_notify_ready(project_id="{project.id}").'
            ),
        }
...
@action(
    name="agent_app_list_projects",
    description=(
        "List the user's Agent App projects: id, name, status, URL, path, "
        "and whether the app is delivered. Call this FIRST whenever the "
        "user refers to a Agent App you don't have a project_id for ('the "
        "app', 'my tracker', 'add it to the agent app') — never ask the "
...
async def agent_app_list_projects(input_data: dict) -> dict:
    ...
    try:
        ...
        def _delivered(project_id: str) -> bool:
            ...
            try:
                ...
                from app.agent_app.lifecycle import has_live_env as _hle
        ...
        return {
            "status": "success",
            "projects": projects,
            "message": f"{len(projects)} Agent App project(s).",
        }
    except Exception as e:
...
@action(
    name="agent_app_notify_ready",
    description=(
        "Gate and boot a Agent App project's DEV environment after a CODE "
    ...
    input_schema={
        "project_id": {
            "type": "string",
            "example": "abc12345",
            "description": "The Agent App project ID (provided in task instruction).",
        },
    },
    output_schema={
        "status": {
        ...
        "message": {
            "type": "string",
            "example": "Agent App abc12345 is now ready at http://localhost:3100",
...
async def agent_app_notify_ready(input_data: dict) -> dict:
    """Launch, verify, and notify browser that a Agent App is ready."""
    ...
    if simulated_mode:
        return {
            "status": "success",
            "message": f"Agent App {project_id} is now ready at http://localhost:3100",
        }
    ...
    try:
        ...
        if not manager:
            return {
                "status": "error",
                "message": "Agent App manager not initialized. Browser adapter may not be running.",
            }
        ...
        _live_exists = False
        try:
            from app.agent_app.lifecycle import live_db_exists as _lde
        ...
        if result["status"] == "success":
            ...
            # Verification is its own visible step: agent_app_walk_verify.
            ...
            _checks = "gate and health checks passed"
            return {
                "status": "success",
                "message": (
                    f"App launched at {url} — {_checks}."
                    f"{_op_note} {env_note}{_dir_note}{spec_note}NOT "
                    "VERIFIED YET: call "
...
@action(
    ...
    description=(
        ...
        "Agent App change completes. Observed defects return the failure "
        "report: fix, relaunch with agent_app_notify_ready, then call this "
        "again. Requires agent_app_notify_ready first (it boots the dev "
    ...
    output_schema={
        ...
        "message": {
            "type": "string",
            "example": "Agent App abc12345 is now ready (5 features walk-verified).",
...
async def agent_app_walk_verify(input_data: dict) -> dict:
    ...
    if input_data.get("simulated_mode"):
        return {
            "status": "success",
            "message": f"Agent App {project_id} verified (simulated).",
        }
    if not project_id:
        return {"status": "error", "message": "project_id is required"}
    ...
    try:
        ...
        from app.agent_app.walk_verify import run_walk_verify
            ...
            if _dev is None:
                return {
                    ...
                    "message": (
                        ...
                        "none exists. Call agent_app_notify_ready first (it "
        ...
        if _is_external and project.status != "running":
            return {
                "status": "error",
                "message": (
                    "The app is not running — call agent_app_notify_ready "
                    "first, then verify."
                ),
            }
        url = f"http://127.0.0.1:{project.port}"
        verify_url = _dev.url if _dev is not None else url
        ...
        # tree, nothing is copied. The instance's dir is the shadow's per-boot
        ...
        try:
            from app.agent_app.walk_verify import describe_scope as _describe_scope
        ...
        # `blocked` is now a TYPED verdict (the verifier set verdict="blocked"
        # with a blocked_reason), not a phrase guessed from prose — so it is
        # trusted directly. A report that is absent or shape-invalid is already
        # classified `unparseable` upstream, which the belt below handles.
            ...
            if _throttles <= 3:
                return {
                    ...
                    "message": (
                        ...
                        "then call agent_app_walk_verify again. Do NOT "
        ...
        if kind == "unparseable":
            ...
            if decision is not None and decision.payload.get("redo") == "verify":
                return {
                    "status": "error",
                    "message": f"{_what}. Call agent_app_walk_verify once more.",
                }
            return {
                "status": "error",
                "message": (
        ...
        if kind == "defects":
            ...
            # The verdict is typed JSON; parse_check_report has already
            # DERIVED the per-feature FAIL lines into `defects` (the exact
            # "- <feature> — FAIL — <observed>" shape the distiller consumes).
            # Those structured lines are the distiller's input — NOT the raw
            # JSON, which carries no FAIL lines and would distill to a bogus
            # 'unstructured-failure' card (the 2026-09-10 fix-loop that never
            # converged). `raw` is kept only for the human-readable record.
        ...
        # (docs/plans/agent-app-unified-lifecycle-plan.md): the real app
        ...
        if promoted.get("status") != "success":
            return {
                ...
                "message": (
                    ...
                    "agent_app_notify_ready and agent_app_walk_verify again."
                ),
                "test_errors": promoted.get("errors", [])[:10],
            }

        await broadcast_agent_app_ready(project_id, url, project.port)
        # No verify-outcome caveat is shown to the user. A clean verdict
        # (pass / incomplete / tooling-blocked) all deliver the app; the old
        # ⚠️ lines ("some parts may not fully work yet", "could not fully test
        # it yet") confused users more than they helped — the app is either
        # delivered or it is not. The verdict is still recorded via
        # report_verify below for the machine's own bookkeeping.
        caveat = ""
...
@action(
    ...
    description=(
        ...
        "invoked). Call during adoption AFTER agent_app_notify_ready and "
    ...
    input_schema={
        "project_id": {
            "type": "string",
            "example": "abc12345",
            "description": "The external Agent App project ID.",
        },
        "op_names": {
            "type": "array",
...
async def agent_app_ops_verify(input_data: dict) -> dict:
    ...
    try:
        ...
        if getattr(project, "project_type", "native") != "external":
            return {
                "status": "error",
                "message": (
                    f"'{project_id}' is a native Agent App — its operations "
...
@action(
    name="agent_app_restart",
    description=(
        "Restart a Agent App project (backend + frontend). "
    ...
    input_schema={
        "project_id": {
            "type": "string",
            "example": "5a58a160",
            "description": "The Agent App project ID (from agent_app_projects.json).",
        },
    },
    output_schema={
        "status": {
        ...
        "message": {
            "type": "string",
            "example": "Agent App '5a58a160' restarted",
            "description": "Status message.",
        },
        "test_errors": {
            "type": "array",
...
async def agent_app_restart(input_data: dict) -> dict:
    """Restart a running Agent App project."""
    project_id = input_data.get("project_id", "")
    simulated_mode = input_data.get("simulated_mode", False)
    ...
    try:
        from app.agent_app import restart_agent_app

        result = await restart_agent_app(project_id)
        return result
    except Exception as e:
        return {
            "status": "error",
...
@action(
    name="agent_app_report_progress",
    description=(
        "Report progress ONLY during Agent App creation (initial build). "
        ...
        "(use send_message for runtime narration, or agent_app_http to read/write data)."
    ),
    default=False,
    mode="CLI",
    action_sets=["agent_app"],
...
async def agent_app_report_progress(input_data: dict) -> dict:
    """Report Agent App creation progress to browser."""
    project_id = input_data.get("project_id", "")
    ...
    try:
        from app.agent_app import broadcast_agent_app_progress, get_agent_app_manager
        ...
        if project and project.status == "running":
            return {
                ...
                "message": (
                    ...
                    "the creation phase. Use send_message to narrate runtime work, or agent_app_http "
                    "to read/write data."
                ),
            }

        success = await broadcast_agent_app_progress(
...
@action(
    ...
    description=(
        ...
        "target is a known Agent App process."
    ),
    default=False,
    mode="CLI",
    action_sets=["agent_app"],
    ...
    input_schema={
        ...
        "path": {
            ...
            "description": "API path on the Agent App backend, starting with '/'. Do NOT include scheme or host.",
        },
        "headers": {
            "type": "object",
...
def agent_app_http(input_data: dict) -> dict:
    """HTTP request scoped to a registered Agent App project's backend."""
    import sys
    import subprocess
    import importlib
    import time
    ...
    try:
        ...
        return {
            ...
            "final_url": "",
            "elapsed_ms": 0,
            "message": f"Agent App manager unavailable: {e}",
        }
    ...
    if not manager:
        return {
            ...
            "final_url": "",
            "elapsed_ms": 0,
            "message": "Agent App manager not initialized.",
        }
    ...
    if _dev_url is None and _mid_arc and method != "GET":
        return {
            ...
            "message": (
                ...
                "change, call agent_app_notify_ready first (it boots the dev "
...
@action(
    ...
    input_schema={
        "project_id": {
            "type": "string",
            "example": "84d93cca",
            "description": "The Agent App project ID (agent_app_list_projects resolves names to ids).",
        },
    },
    output_schema={
        "status": {"type": "string", "example": "success"},
...
def agent_app_usage(input_data: dict) -> dict:
    """Return the same operating note the project's dedicated session gets."""
    if input_data.get("simulated_mode", False):
        return {
            "status": "success",
            "usage": "[INTERACTING WITH AGENT APP: test123]",
            "message": "",
        }
    ...
    try:
        ...
        if not manager or not manager.get_project(project_id):
            return {
                ...
                "message": (
                    ...
                    "agent_app_list_projects to resolve the id."
                ),
            }
        ...
        return {
            "status": "success",
            "usage": AgentBase._build_agent_app_note(project_id),
            "message": "",
        }
    except Exception as e:
        return {
...
@action(
    name="agent_app_marketplace_list",
    description=(
        "List the Agent App marketplace catalogue: pre-built apps the user "
...
async def agent_app_marketplace_list(input_data: dict) -> dict:
    ...
    try:
        ...
        return {
            ...
            "message": (
                ...
                'agent_app_marketplace_install(app_id="<id>").'
            ),
        }
    except Exception as e:
        return {
...
@action(
    name="agent_app_approve_triggers",
    description=(
        "Approve a Agent App app's declared agent triggers (its triggers.json "
    ...
    input_schema={
        "project_id": {
            "type": "string",
            "example": "84d93cca",
            "description": "The Agent App project whose triggers the user approved.",
        },
    },
    output_schema={
        "status": {"type": "string", "example": "success"},
...
def agent_app_approve_triggers(input_data: dict) -> dict:
...
@action(
    name="agent_app_marketplace_install",
    description=(
        "Install a pre-built Agent App app from the marketplace by id "
        "(resolve ids with agent_app_marketplace_list). Downloads the app, "
        "registers it as a project, and runs the full launch pipeline. "
        "Inside a Agent App build session, the install ADOPTS the current "
...
async def agent_app_marketplace_install(input_data: dict) -> dict:
    ...
    try:
                        ...
                        if _mf.get("marketplaceAppId") == app_id:
                            return {
                                ...
                                "message": (
                                    ...
                                    "apply them now (edit → "
                                    "agent_app_notify_ready → "
                                    "agent_app_walk_verify); otherwise the "
        ...
        try:
            await broadcast_agent_app_created(project)
            live = manager.get_project(project_id)
            if live is not None and live.port:
                await broadcast_agent_app_ready(project_id, url, live.port)
        except Exception:
            pass
        ...
        if adopt_id and will_adapt:
            return {
                ...
                "message": (
                    ...
                    "(agent_app_notify_ready will boot your changes in the "
                    "dev environment), then agent_app_walk_verify to deploy "
...
@action(
    name="agent_app_import_zip",
    description=(
        "Import a Agent App project from an exported ZIP file (round-trip "
        ...
        "agent_app_notify_ready, then agent_app_walk_verify. Only Living "
    ...
    input_schema={
        "zip_path": {
            ...
            "description": "Absolute path to the exported Agent App ZIP.",
        },
        "name": {
            "type": "string",
...
async def agent_app_import_zip(input_data: dict) -> dict:
    """Import an exported Agent App ZIP as a new registered project."""
    zip_path = (input_data.get("zip_path") or "").strip()
    if input_data.get("simulated_mode"):
        return {
            "status": "success",
            "project_id": "abc12345",
            "project_path": "/workspace/agent_app/imported_abc12345",
    ...
    try:
        from app.agent_app import broadcast_agent_app_created, get_agent_app_manager
        ...
        return {
            ...
            "message": (
                ...
                f'Now launch it: agent_app_notify_ready(project_id="{project.id}"), '
                f'then agent_app_walk_verify(project_id="{project.id}").'
            ),
        }
...
@action(
    name="agent_app_import",
    description=(
        "Import a Agent App project from ANY source: an exported ZIP "
        ...
        "yourself. Only Agent App projects import (a foreign app/repo is "
...
async def agent_app_import(input_data: dict) -> dict:
    """Import a Agent App project from zip/folder/git and queue its verify run."""
    ...
    try:
        ...
        try:
            ...
            _dispatched = await manager.start_development_run(
                ...
                trigger_source=_TS.AGENT_APP_IMPORT,
                workflow_skill=(
                    "agent-app-importer" if _is_ext else "agent-app-modify"
                ),
...
@action(
    name="agent_app_convert",
    description=(
        "REBUILD a foreign (non-Agent-App) app as a Agent App: scaffolds a "
        "fresh Agent App project, ships the original source (zip / folder / git "
        ...
        "'imported' but agent_app_import rejected it as not a Agent App — this is a "
        ...
        "actual Agent App projects use agent_app_import instead."
    ),
    default=False,
    mode="CLI",
    action_sets=["agent_app"],
...
async def agent_app_convert(input_data: dict) -> dict:
    ...
    if input_data.get("simulated_mode"):
        return {
            "status": "success",
            "project_id": "abc12345",
            "project_path": "/workspace/agent_app/converted_abc12345",
    ...
    try:
        ...
        _next = (
            ...
            if _dispatched
                ...
                "dispatched. Build it via the agent-app-creator workflow "
