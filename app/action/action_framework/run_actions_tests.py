...
def run_tests():
    ...
    logger.info(
        f"-> Found {len(testable_actions)} testable actions. Starting execution...\n"
    )
    ...
    for i, action_impl in enumerate(testable_actions, 1):
        ...
        try:
            ...
            if result is None:
                logger.error("❌ TEST FAILED. Action returned None.")
                fail_count += 1
                ...
                if status in ("success", "ok") or (status is None and len(result) > 0):
                    logger.info("✅ TEST PASSED. Result output:")
                    # Pretty print the result dict nicely
                    logger.info(json.dumps(result, indent=2))
                    success_count += 1
                elif status == "error":
                    logger.error("❌ TEST FAILED. Action returned error status.")
                    ...
                    if status in ("ignored", "completed", "queued"):
                        ...
                        logger.error(
                            "❌ TEST FAILED. Action finished but status was not 'success' or 'ok'."
                        )
                        logger.error(f"Output: {result}")
                        fail_count += 1
            else:
                logger.error("❌ TEST FAILED. Action did not return a dict.")
