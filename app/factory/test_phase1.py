...
with tempfile.TemporaryDirectory() as td:
    DISPATCHED.clear(); CHAT.clear()
    host = make_host(Path(td))
    host.report_launch_success("testproj")
    d = host.report_verify("testproj", "defects",
...
with tempfile.TemporaryDirectory() as td:
    ...
    host = make_host(Path(td))
    host.report_launch_success("testproj")
    d = host.report_verify("testproj", "pass", url="http://127.0.0.1:3100",
                           verified=["feature a", "feature b"], caveat="")
    assert d.next_state == "done"
    assert CHAT and "ready at http://127.0.0.1:3100" in CHAT[-1] and "2 feature" in CHAT[-1]
    host.on_run_end("testproj", {})
...
with tempfile.TemporaryDirectory() as td:
    ...
    assert len(DISPATCHED) == 2, "queued mission must not be cleared by an unrelated run-end"
