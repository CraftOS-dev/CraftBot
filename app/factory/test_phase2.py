...
assert any("ERR_CONNECTION_REFUSED" in e for e in cards[0].evidence), (
    "URL+cause must be quoted"
)
assert "node cli.ts run /w/proj refresh-stories" == cards[0].repro
blob = cards[0].render()
assert (
    "Vite" not in blob and "vite" not in blob
)  # the hallucination is not utterable from evidence
...
assert len(cards) == 1
assert "cannot be blank" in cards[0].candidate_cause, (
    "server evidence must drive the cause"
)
...
cards = distill(
    "- Something — FAIL — it broke | expected: works", server_log="", console_lines=[]
)
