...
class PromptSanitizer:
    ...
    def sanitize_structured_data(
        ...
        for key, value in data.items():
            if isinstance(value, str):
                ...
                sanitized[key] = [
                    PromptSanitizer.sanitize_user_message(str(v))
                    if isinstance(v, str)
                    else v
                    for v in value
                ]
    ...
    def create_safe_context_block(
        context: dict[str, str], block_name: str = "context"
    ) -> str:
        """
...
def example_safe_routing_prompt(
    ...
    safe_item_content = PromptSanitizer.sanitize_user_message(
        item_content, max_length=1000
    )
    safe_platform = PromptSanitizer.sanitize_user_message(
        source_platform, max_length=50
    )
