...
resolve, then require the target to be a strict descendant of the Living UI
workspace.
"""
...
def _assert_inside_living_root(target: Path, living_root: Path, verb: str) -> None:
    """Refuse to touch anything that is not strictly inside the Living UI
    ...
    if root not in resolved.parents:
        raise ValueError(
            f"refusing to {verb} {resolved} — outside the Living UI workspace {root}"
        )
...
def snapshot_pb_data(src_pb_data: Path, dest: Path, living_root: Path) -> None:
    ...
    logger.info(
        f"[LIVING_UI:PB_DATA] snapshot {src_pb_data} -> {dest} "
...
def restore_pb_data(snapshot_dir: Path, pb_data: Path, living_root: Path) -> None:
    ...
    logger.info(f"[LIVING_UI:PB_DATA] restored {pb_data} from {snapshot_dir}")
