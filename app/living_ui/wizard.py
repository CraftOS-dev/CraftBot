"""Living UI creation wizard — pre-build requirement gathering.
...
Attachments upload to a staging area (living_ui/_staging/wizard/<id>)
...
def staging_root(living_ui_dir: Path) -> Path:
    return Path(living_ui_dir) / "_staging" / "wizard"


def staging_dir(living_ui_dir: Path, wizard_id: str) -> Path:
    ...
    d = staging_root(living_ui_dir) / wizard_id
    d.mkdir(parents=True, exist_ok=True)
    return d


def list_staged(living_ui_dir: Path, wizard_id: str) -> List[Path]:
    d = staging_root(living_ui_dir) / wizard_id
    if not d.is_dir():
        return []
...
def sweep_stale_staging(living_ui_dir: Path) -> None:
    """Remove abandoned wizard staging folders. Best-effort."""
    root = staging_root(living_ui_dir)
    try:
        if not root.is_dir():
            return
...
def move_staging_into_project(
    living_ui_dir: Path, wizard_id: str, project_path: Path
) -> Dict[str, Any]:
...
async def describe_staged_images(living_ui_dir: Path, wizard_id: str) -> List[str]:
    ...
    if IAI.vlm_interface is None:
        return []
    notes: List[str] = []
    for f in list_staged(living_ui_dir, wizard_id):
