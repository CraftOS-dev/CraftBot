"""Agent App build-event pipeline — the construction dock's data source.

Derives structured "the app is being built" events from actions the agent
already performs (write_file / stream_edit / agent_app_scaffold /
agent_app_notify_ready). The agent is NEVER asked to narrate progress: events
...
def record_todo_transitions(project_id: str, todos: List[Dict[str, Any]]) -> None:
    ...
    try:
        ...
        logger.debug(f"[AGENT_APP:BUILD_EVENTS] todo transitions failed: {exc}")
...
def make_action_hooks():
    ...
    def on_action_end(run_id, action, outputs, status, parent_id, ended_at):
        try:
            ...
            logger.debug(f"[AGENT_APP:BUILD_EVENTS] classify failed: {exc}")
