...
UI in {project.path}/frontend/src/app/."""
    ...
    async def start_tunnel(
        ...
        # Reap tunnels a previous CraftBot run left behind — by the exact
        # pid+start-time we recorded when starting them, never by process
        # name: a user's own cloudflared (e.g. a production tunnel) is not
        # ours to kill. Tunnels other projects hold right now are skipped.
        held = {
            p.tunnel_process.pid
            for p in self.projects.values()
            if p.tunnel_process is not None
        }
        ledger = get_ledger()
        for entry in ledger.entries(ROLE_TUNNEL):
            if entry.pid not in held:
                ledger.kill_entry(entry)
        ...
        get_ledger().register(
            proc.pid, ROLE_TUNNEL, owner=project.id, label=origin_url
        )
