"""Agent App runner — thin adapter between CraftBot and the agent-app
...
class AgentAppRunnerUnavailable(RuntimeError):
    """Node or the agent-app workspace is missing."""
...
class AgentAppRunner:
    ...
    def ensure_available(self) -> None:
        ...
        if rt is None:
            ...
            raise AgentAppRunnerUnavailable(
                f"Node.js >= {MIN_NODE_MAJOR} is required to build Agent Apps "
        ...
        if not self.cli_path.exists():
            raise AgentAppRunnerUnavailable(
                f"agent-app workspace not found at {self.workspace_dir}"
            )
    ...
    async def adapter_sync(self, project_dir: Path) -> None:
        ...
        try:
            ...
            logger.warning(f"[AGENT_APP] adapter-sync skipped: {e}")
            return
        code, out = await self._run(
        ...
        if code != 0:
            logger.warning(
                f"[AGENT_APP] adapter-sync failed for {project_dir.name} "
    ...
    async def ensure_superuser(self, project_dir: Path) -> None:
        ...
        try:
            ...
            logger.warning(f"[AGENT_APP] could not persist .superuser: {e}")
    ...
    def ensure_agent_token(self, project_dir: Path) -> str:
        ...
        try:
            ...
            logger.warning(f"[AGENT_APP] could not persist .agent-token: {e}")
        return token
    ...
    async def start(
        ...
        if bridge_token:
            ...
            logger.info(
                f"[AGENT_APP] bridge env injected: URL=http://localhost:{bridge_port}, token={bridge_token[:8]}..."
            )
        else:
            logger.warning(
