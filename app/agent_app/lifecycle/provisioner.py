...
agent_app/_staging/project/<id>/ with its identity rewritten for a hidden
...
from app.agent_app.lifecycle.environment import DevInstance
...
_COPY_FILES = ("manifest.json", "operations.json", "triggers.json", "AGENT_APP.md")
...
_SYNC_FILES = ("operations.json", "triggers.json", "AGENT_APP.md")
...
class DevProvisioner:
    ...
    def __init__(self, agent_app_dir: Path, runner) -> None:
        ...
        self.root = self.agent_app_dir / "_staging" / "project"
        self.runner = runner
    ...
    async def create_copy(self, project) -> DevInstance:
        ...
        if not (src / "manifest.json").exists():
            raise FileNotFoundError(f"not a Agent App project: {src}")
        ...
        logger.info(
            f"[AGENT_APP:DEV] created dev copy of {project.id} at "
            f"{dev_dir} (port {port})"
        )
        return instance
    ...
    def sync_code(self, project, dev_dir: Path) -> None:
        ...
        for rel in _SYNC_PKG:
            ...
            if s.exists() and (not d.exists() or s.read_bytes() != d.read_bytes()):
                ...
                if nm.is_dir():
                    logger.info(
                        "[AGENT_APP:DEV] package.json changed — "
    ...
    def destroy(self, project_id: str, record: Optional[Dict[str, Any]]) -> None:
        ...
        if dev_dir.exists():
            try:
                self._guarded_rmtree(dev_dir)
                logger.info(f"[AGENT_APP:DEV] destroyed dev copy of {project_id}")
            except Exception as e:
                logger.warning(f"[AGENT_APP:DEV] failed to delete {dev_dir}: {e}")
        ...
        if self.root.exists():
            for leftover in self.root.iterdir():
                try:
                    ...
                    reaped += 1
                    logger.info(f"[AGENT_APP:DEV] reaped leftover {leftover.name}")
                except Exception as e:
                    logger.warning(f"[AGENT_APP:DEV] failed to reap {leftover}: {e}")
        return reaped

    # ── internals ──────────────────────────────────────────────────────────
    def _guarded_rmtree(self, target: Path) -> None:
        """Only ever delete inside agent_app/_staging/ — the same
        ...
        dev_root = (self.agent_app_dir / "_staging").resolve()
    ...
    def _kill(self, process=None, pid: Optional[int] = None) -> None:
        try:
            ...
            logger.warning(f"[AGENT_APP:DEV] kill failed: {e}")
