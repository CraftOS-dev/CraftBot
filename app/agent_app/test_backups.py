"""Acceptance tests for Agent App backups — store, capture (both paths),
...
Spec: docs/plans/agent-app-backups-requirements.md / -plan.md.

Style follows test_data_safety.py: a module-level assert script with
section prints — run directly:

    PYTHONPATH=. python app/agent_app/test_backups.py
"""
...
from app.agent_app.lifecycle.backups import (
    _NAME_RE,
    PRE_PROMOTE_KEEP,
    BackupService,
    BackupStore,
...
with tempfile.TemporaryDirectory() as tmp:
    mgr = AgentAppManager(workspace_root=Path(tmp))
    project = _make_project(mgr.agent_app_dir, "sched001")
    lp = AgentAppProject(
...
with tempfile.TemporaryDirectory() as tmp:
    mgr = AgentAppManager(workspace_root=Path(tmp))
    project = _make_project(mgr.agent_app_dir, "promo001")
    lp = AgentAppProject(
...
with tempfile.TemporaryDirectory() as tmp:
    mgr = AgentAppManager(workspace_root=Path(tmp))
    project = _make_project(mgr.agent_app_dir, "rest0001")
    lp = AgentAppProject(
        id="rest0001",
        name="r",
        description="",
    ...
    async def _t5():
        ...
        dead_dir = mgr.agent_app_dir / "app_dead0001"
...
with tempfile.TemporaryDirectory() as tmp:
    ...
    project = _make_project(mgr.agent_app_dir, "keep0001")
    lp = AgentAppProject(
    ...
    project2 = _make_project(mgr.agent_app_dir, "kill0001")
    lp2 = AgentAppProject(
...
# ── §7 settings surface ────────────────────────────────────────────────────
from app.ui_layer.settings.agent_app_settings import (  # noqa: E402
    get_agent_app_projects,
    update_project_setting,
)

with tempfile.TemporaryDirectory() as tmp:
    mgr = AgentAppManager(workspace_root=Path(tmp))
    project = _make_project(mgr.agent_app_dir, "sett0001")
    lp = AgentAppProject(
...
def _pinned_pb_binary() -> Path:
    ...
    cache = _os.environ.get("AGENT_APP_PB_CACHE")
    if cache:
        root = Path(cache)
    elif _os.name == "nt":
        root = Path(_os.environ["LOCALAPPDATA"]) / "craftos-agent-app" / "pb"
    else:
