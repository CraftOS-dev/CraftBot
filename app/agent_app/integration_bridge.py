# -*- coding: utf-8 -*-
"""
Integration Bridge — proxy for Agent App ↔ External API calls.

Agent App backends call CraftBot via this bridge to make authenticated
...
class IntegrationBridge:
    """
    HTTP proxy that lets Agent App backends make authenticated API calls
    to external services through CraftBot.

    Flow:
        Agent App Backend → POST /api/integrations/proxy → CraftBot
        CraftBot validates token, injects auth, forwards to external API.
    """

    def __init__(self, manager: "AgentAppManager"):
    ...
    async def _handle_action(self, request: web.Request) -> web.Response:  # noqa: C901
        """Execute one of CRAFTBOT'S OWN integration actions for a Agent App.
    ...
    def _project_action_grants(self, project_id: str, action_name: str) -> tuple:
        ...
        if not isinstance(declared, list) or action_name not in declared:
            return False, (
                ...
                "relaunch with agent_app_notify_ready."
            )
        return True, ""

    async def _handle_agent_request(self, request: web.Request) -> web.Response:
        """A Agent App's nudge hook reports a new agent_requests row (spec
                ...
                return web.json_response(
                    {
                        "error": (
                            ...
                            "Ask them to approve via agent_app_approve_triggers."
                        )
                    },
                    status=403,
                )
    ...
    def _project_grants(self, project_id: str, integration: str) -> tuple:
        ...
        if not isinstance(declared, list):
            return False, (
                ...
                "a literal id and relaunch with agent_app_notify_ready."
            )
