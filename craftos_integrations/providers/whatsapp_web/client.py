...
class WhatsAppWebClient(BasePlatformClient):
    ...
    def _store_updated_credential(self, updated: WhatsAppWebCredential) -> None:
        ...
        event. Bound multi-account clients (the provider binding)
        override this to route through the account store; the base client
        keeps it in memory only — whatsapp_web.json is never
        written anymore (session-durability plan §2.8)."""
        self._cred = updated
    ...
    def _session_identity(self) -> str:
        ...
        or, for an unbound client, the credential's owner phone."""
...
# whatsapp_web.json is never written (session-durability plan §2.8).
