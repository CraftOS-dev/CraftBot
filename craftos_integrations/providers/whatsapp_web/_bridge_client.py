...
def _node_runtime():
    """The host's resolved Node runtime, or None.

    Injected as ``ConfigStore.extras["node_runtime"]`` — a module or object
    exposing ``node_cmd()`` / ``npm_cmd()`` / ``child_env()``. The package must
    not import the host (tests/integrations/test_isolation.py), and without it
    the bare-name fallbacks below behave exactly as before.
    """
    return ConfigStore.extras.get("node_runtime")
...
class WhatsAppBridge:
    ...
    async def start(self) -> None:
        ...
        npm_cmd = (runtime.npm_cmd() if runtime else None) or (
            "npm.cmd" if os.name == "nt" else "npm"
        )
        node_cmd = (runtime.node_cmd() if runtime else None) or (
            "node.exe" if os.name == "nt" else "node"
        )
        bridge_env = runtime.child_env() if runtime else None
        ...
        try:
            while self._running and self._process and self._process.stdout:
                try:
                    ...
                    # survives.
                    logger.error(f"[WA-Bridge] Oversized stdout line dropped: {e}")
                    try:
                        while True:
    ...
    try:
        ...
        for child in root.iterdir():
            ...
            try:
                if (
                    marker.exists()
                    and marker.read_text(encoding="utf-8").strip() == identity
                ):
                    out.append(child)
            except OSError:
                continue
...
def max_whatsapp_accounts() -> int:
    ...
    try:
        ...
        from .client import WhatsAppWebConfig, _whatsapp_web_config_file
...
def _ensure_layout_migrated() -> None:
    ...
    migration is gone with the system — single-account wwebjs
...
def get_whatsapp_bridge(identity: str) -> WhatsAppBridge:
    ...
    passes an identity — the identity-less resolution is gone."""
