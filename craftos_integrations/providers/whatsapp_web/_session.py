...
class WhatsAppSession:
        ...
        if bridge is not None:
            ...
            if bridge.is_running:
                try:
                    ...
                    logger.warning(f"[WA-Session] {self.identity}: stop error: {e}")
    ...
    async def _launch(self) -> None:
        ...
        try:
            ...
            await bridge.start()
            self._start_supervisor(bridge)
            event_type, _ = await bridge.wait_for_qr_or_ready(timeout=self.LAUNCH_WAIT)
            if self._stopping:
                return
            if event_type == "ready":
                self._failures = 0
                ...
                try:
                    ...
                    if misses >= self.HEARTBEAT_MISSES:
                        ...
                        try:
                            ...
                            pass
                        if not self._stopping:
                            self._register_failure("heartbeat: bridge process hung")
                        return
        except asyncio.CancelledError:
            pass
        finally:
    ...
    async def _on_bridge_event(self, event: str, data: Dict[str, Any]) -> None:
        ...
        if subscriber is not None:
            try:
                ...
                logger.warning(
                    f"[WA-Session] {self.identity}: subscriber error on '{event}': {e}"
                )
        ...
        try:
            while self.state in (FLOW_QR_READY, FLOW_SCANNED):
                ...
                if now - self.last_poll > self.ABANDON_AFTER:
                    ...
                    await self.cancel(reason="QR session abandoned (no polling).")
                    return
                if (
                    self.state == FLOW_QR_READY
