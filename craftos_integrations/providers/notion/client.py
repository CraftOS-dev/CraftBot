...
# OAuth flow for notion, at module scope so the v2 provider can import it
# without reaching through the Handler class (removal plan B3).
NOTION_OAUTH = OAuthFlow(
...
class NotionClient(BasePlatformClient):
    ...
    def update_database(
        ...
        return _notion_call(
            "PATCH", f"/databases/{database_id}", self._headers(), json=payload
        )
    ...
    def update_block(
        self, block_id: str, block_update: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        """
        return _notion_call(
            "PATCH", f"/blocks/{block_id}", self._headers(), json=block_update
        )
    ...
    def create_comment(
        ...
        if len(targets) != 1:
            return {
                "error": {
                    "validation": "Provide exactly one of parent_page_id, parent_block_id, discussion_id"
                }
            }
        if parent_page_id:
    ...
    def send_file_upload(
        self, file_upload_id: str, file_path: str, part_number: Optional[int] = None
    ) -> Dict[str, Any]:
