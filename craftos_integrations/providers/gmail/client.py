...
# signatures, Authentication-Results, SPF, List-Unsubscribe JWTs, Feedback-ID,
# X-* vendor tags) that the agent never acts on but always pays for — it was
# ~55% of every get_gmail payload before this filter. Matched case-insensitively
    ...
    def get_email(self, message_id: str, full_body: bool = False) -> Result:
        ...
        def _shape(msg):
            email_info: Dict[str, Any] = {
                ...
                # 100% noise to the agent: Received chains, ARC-Seal /
                # ARC-Message-Signature / DKIM-Signature base64 blobs,
                # Authentication-Results, List-Unsubscribe JWTs, Feedback-ID. Filter
                # here so both formats return the same small, useful header set.
                ...
                "labels": [
                    {
                        ...
                        "name": lbl.get("name"),
                        "type": lbl.get("type"),
                        "messageListVisibility": lbl.get("messageListVisibility"),
                        "labelListVisibility": lbl.get("labelListVisibility"),
                    }
