...
class GitHubClient(BasePlatformClient):
    ...
    async def create_repo(self, name: str, description: str = "", private: bool = False, auto_init: bool = False) -> Result:
        payload: Dict[str, Any] = {"name": name, "private": private, "auto_init": auto_init}
        if description:
            payload["description"] = description
        return await arequest(
            "POST", f"{GITHUB_API}/user/repos",
            headers=self._headers(),
            json=payload,
            expected=(201,),
            transform=lambda d: {"full_name": d.get("full_name"), "html_url": d.get("html_url"), "private": d.get("private")},
        )

    async def update_repo(self, owner_repo: str, name: Optional[str] = None, description: Optional[str] = None,
        ...
        return await arequest(
            ...
            json=payload,
            expected=(200,),
            transform=lambda d: {"full_name": d.get("full_name"), "html_url": d.get("html_url")},
        )
    ...
    async def fork_repo(self, owner_repo: str, organization: Optional[str] = None, name: Optional[str] = None,
                        default_branch_only: bool = False) -> Result:
        payload: Dict[str, Any] = {"default_branch_only": default_branch_only}
        if organization: payload["organization"] = organization
        if name: payload["name"] = name
        return await arequest(
            "POST", f"{GITHUB_API}/repos/{owner_repo}/forks",
            headers=self._headers(),
            json=payload,
            expected=(202,),
            transform=lambda d: {"full_name": d.get("full_name"), "html_url": d.get("html_url"), "default_branch": d.get("default_branch")},
        )

    async def list_forks(self, owner_repo: str, per_page: int = 30) -> Result:
        return await arequest(
            "GET", f"{GITHUB_API}/repos/{owner_repo}/forks",
            headers=self._headers(),
            params={"per_page": per_page},
            expected=(200,),
            transform=lambda d: {"forks": [{"full_name": f.get("full_name"), "html_url": f.get("html_url"), "owner": f.get("owner", {}).get("login")} for f in d]},
        )

    async def list_collaborators(self, owner_repo: str, per_page: int = 30) -> Result:
        return await arequest(
            "GET", f"{GITHUB_API}/repos/{owner_repo}/collaborators",
            headers=self._headers(),
            params={"per_page": per_page},
            expected=(200,),
            transform=lambda d: {"collaborators": [{"login": u.get("login"), "permissions": u.get("permissions", {})} for u in d]},
        )

    async def add_collaborator(self, owner_repo: str, username: str, permission: str = "push") -> Result:
        return await arequest(
            "PUT", f"{GITHUB_API}/repos/{owner_repo}/collaborators/{username}",
            headers=self._headers(),
            json={"permission": permission},
            expected=(201, 204),
            transform=lambda d: {"added": True, "username": username, "invitation_id": (d or {}).get("id")},
        )

    async def remove_collaborator(self, owner_repo: str, username: str) -> Result:
        return await arequest(
            "DELETE", f"{GITHUB_API}/repos/{owner_repo}/collaborators/{username}",
            headers=self._headers(),
            expected=(204,),
    ...
    async def get_readme(self, owner_repo: str, ref: Optional[str] = None) -> Result:
        ...
        return await arequest(
            "GET", f"{GITHUB_API}/repos/{owner_repo}/readme",
            headers=self._headers(),
            params=params,
            expected=(200,),
            transform=lambda d: {"name": d.get("name"), "path": d.get("path"), "download_url": d.get("download_url"), "content_b64": d.get("content"), "encoding": d.get("encoding")},
        )

    async def list_topics(self, owner_repo: str) -> Result:
        return await arequest(
            "GET", f"{GITHUB_API}/repos/{owner_repo}/topics",
            headers=self._headers(),
            expected=(200,),
    ...
    async def set_topics(self, owner_repo: str, names: List[str]) -> Result:
        return await arequest(
            "PUT", f"{GITHUB_API}/repos/{owner_repo}/topics",
            headers=self._headers(),
            json={"names": names},
            expected=(200,),
            transform=lambda d: {"topics": d.get("names", [])},
    ...
    async def get_file(self, owner_repo: str, path: str, ref: Optional[str] = None) -> Result:
        params = {"ref": ref} if ref else None
        return await arequest(
            "GET", f"{GITHUB_API}/repos/{owner_repo}/contents/{path}",
            headers=self._headers(),
            params=params,
            expected=(200,),
        )

    async def create_or_update_file(self, owner_repo: str, path: str, message: str, content_b64: str,
                                    sha: Optional[str] = None, branch: Optional[str] = None) -> Result:
        payload: Dict[str, Any] = {"message": message, "content": content_b64}
        if sha: payload["sha"] = sha
        if branch: payload["branch"] = branch
        return await arequest(
            "PUT", f"{GITHUB_API}/repos/{owner_repo}/contents/{path}",
            headers=self._headers(),
            json=payload,
            expected=(200, 201),
            transform=lambda d: {
    ...
    async def delete_file(self, owner_repo: str, path: str, message: str, sha: str,
                          branch: Optional[str] = None) -> Result:
        payload: Dict[str, Any] = {"message": message, "sha": sha}
        if branch: payload["branch"] = branch
        return await arequest(
            "DELETE", f"{GITHUB_API}/repos/{owner_repo}/contents/{path}",
            headers=self._headers(),
            json=payload,
            expected=(200,),
            transform=lambda d: {"commit_sha": d.get("commit", {}).get("sha"), "deleted": True},
        )
    ...
    async def list_branches(self, owner_repo: str, per_page: int = 30) -> Result:
        return await arequest(
            "GET", f"{GITHUB_API}/repos/{owner_repo}/branches",
            headers=self._headers(),
            params={"per_page": per_page},
            expected=(200,),
            transform=lambda d: {"branches": [{"name": b.get("name"), "sha": b.get("commit", {}).get("sha"), "protected": b.get("protected")} for b in d]},
        )

    async def get_branch(self, owner_repo: str, branch: str) -> Result:
        return await arequest(
            "GET", f"{GITHUB_API}/repos/{owner_repo}/branches/{branch}",
            headers=self._headers(),
            expected=(200,),
            transform=lambda d: {"name": d.get("name"), "sha": d.get("commit", {}).get("sha"), "protected": d.get("protected")},
        )

    async def create_branch(self, owner_repo: str, branch: str, from_sha: str) -> Result:
        return await arequest(
            "POST", f"{GITHUB_API}/repos/{owner_repo}/git/refs",
            headers=self._headers(),
            json={"ref": f"refs/heads/{branch}", "sha": from_sha},
            expected=(201,),
            transform=lambda d: {"ref": d.get("ref"), "sha": d.get("object", {}).get("sha")},
        )

    async def delete_branch(self, owner_repo: str, branch: str) -> Result:
        return await arequest(
            "DELETE", f"{GITHUB_API}/repos/{owner_repo}/git/refs/heads/{branch}",
            headers=self._headers(),
            expected=(204,),
    ...
    async def list_commits(self, owner_repo: str, sha: Optional[str] = None, path: Optional[str] = None,
                           author: Optional[str] = None, per_page: int = 30) -> Result:
        params: Dict[str, Any] = {"per_page": per_page}
        if sha: params["sha"] = sha
        if path: params["path"] = path
        if author: params["author"] = author
        return await arequest(
            "GET", f"{GITHUB_API}/repos/{owner_repo}/commits",
            headers=self._headers(),
            params=params,
            expected=(200,),
            transform=lambda d: {"commits": [{
                "sha": c.get("sha"),
    ...
    async def get_commit(self, owner_repo: str, sha: str) -> Result:
        return await arequest(
            "GET", f"{GITHUB_API}/repos/{owner_repo}/commits/{sha}",
            headers=self._headers(),
            expected=(200,),
        )
    ...
    async def compare_commits(self, owner_repo: str, base: str, head: str) -> Result:
        return await arequest(
            "GET", f"{GITHUB_API}/repos/{owner_repo}/compare/{base}...{head}",
            headers=self._headers(),
            expected=(200,),
            transform=lambda d: {
                "status": d.get("status"),
                ...
                "files": [{"filename": f.get("filename"), "status": f.get("status"), "additions": f.get("additions"), "deletions": f.get("deletions")} for f in d.get("files", [])],
            },
        )
    ...
    async def get_pull_request(self, owner_repo: str, number: int) -> Result:
        return await arequest(
            "GET", f"{GITHUB_API}/repos/{owner_repo}/pulls/{number}",
            headers=self._headers(),
            expected=(200,),
        )

    async def create_pull_request(self, owner_repo: str, title: str, head: str, base: str,
                                  body: str = "", draft: bool = False,
                                  maintainer_can_modify: bool = True) -> Result:
        payload: Dict[str, Any] = {
        ...
        return await arequest(
            "POST", f"{GITHUB_API}/repos/{owner_repo}/pulls",
            headers=self._headers(),
            json=payload,
            expected=(201,),
            transform=lambda d: {"number": d.get("number"), "html_url": d.get("html_url"), "title": d.get("title"), "state": d.get("state")},
        )

    async def update_pull_request(self, owner_repo: str, number: int, title: Optional[str] = None,
                                  body: Optional[str] = None, state: Optional[str] = None,
                                  base: Optional[str] = None) -> Result:
        payload: Dict[str, Any] = {}
        if title is not None: payload["title"] = title
        if body is not None: payload["body"] = body
        if state is not None: payload["state"] = state
        if base is not None: payload["base"] = base
        return await arequest(
            "PATCH", f"{GITHUB_API}/repos/{owner_repo}/pulls/{number}",
            headers=self._headers(),
            json=payload,
            expected=(200,),
            transform=lambda d: {"number": d.get("number"), "state": d.get("state"), "html_url": d.get("html_url")},
        )

    async def merge_pull_request(self, owner_repo: str, number: int, commit_title: Optional[str] = None,
                                 commit_message: Optional[str] = None, sha: Optional[str] = None,
                                 merge_method: str = "merge") -> Result:
        payload: Dict[str, Any] = {"merge_method": merge_method}
        if commit_title: payload["commit_title"] = commit_title
        if commit_message: payload["commit_message"] = commit_message
        if sha: payload["sha"] = sha
        return await arequest(
            "PUT", f"{GITHUB_API}/repos/{owner_repo}/pulls/{number}/merge",
            headers=self._headers(),
            json=payload,
            expected=(200,),
            transform=lambda d: {"merged": d.get("merged"), "sha": d.get("sha"), "message": d.get("message")},
        )

    async def list_pr_files(self, owner_repo: str, number: int, per_page: int = 30) -> Result:
        return await arequest(
            "GET", f"{GITHUB_API}/repos/{owner_repo}/pulls/{number}/files",
            headers=self._headers(),
            params={"per_page": per_page},
            expected=(200,),
            transform=lambda d: {"files": [{"filename": f.get("filename"), "status": f.get("status"), "additions": f.get("additions"), "deletions": f.get("deletions"), "patch": (f.get("patch") or "")[:500]} for f in d]},
        )

    async def list_pr_commits(self, owner_repo: str, number: int, per_page: int = 30) -> Result:
        return await arequest(
            "GET", f"{GITHUB_API}/repos/{owner_repo}/pulls/{number}/commits",
            headers=self._headers(),
            params={"per_page": per_page},
            expected=(200,),
            transform=lambda d: {"commits": [{"sha": c.get("sha"), "message": (c.get("commit", {}).get("message") or "").split("\n")[0], "author": c.get("commit", {}).get("author", {}).get("name")} for c in d]},
        )

    async def request_pr_reviewers(self, owner_repo: str, number: int,
                                   reviewers: Optional[List[str]] = None,
                                   team_reviewers: Optional[List[str]] = None) -> Result:
        payload: Dict[str, Any] = {}
        if reviewers: payload["reviewers"] = reviewers
        if team_reviewers: payload["team_reviewers"] = team_reviewers
        return await arequest(
            "POST", f"{GITHUB_API}/repos/{owner_repo}/pulls/{number}/requested_reviewers",
            headers=self._headers(),
            json=payload,
            expected=(201,),
            transform=lambda d: {"requested": True, "reviewers": [u.get("login") for u in d.get("requested_reviewers", [])]},
        )

    async def remove_pr_reviewers(self, owner_repo: str, number: int,
                                  reviewers: Optional[List[str]] = None,
                                  team_reviewers: Optional[List[str]] = None) -> Result:
        payload: Dict[str, Any] = {}
        if reviewers: payload["reviewers"] = reviewers
        if team_reviewers: payload["team_reviewers"] = team_reviewers
        return await arequest(
            "DELETE", f"{GITHUB_API}/repos/{owner_repo}/pulls/{number}/requested_reviewers",
            headers=self._headers(),
            json=payload,
            expected=(200,),
            transform=lambda _d: {"removed": True, "reviewers": reviewers or [], "team_reviewers": team_reviewers or []},
        )

    async def create_pr_review(self, owner_repo: str, number: int, body: str = "",
                               event: Optional[str] = None,
                               comments: Optional[List[Dict[str, Any]]] = None) -> Result:
        payload: Dict[str, Any] = {}
        if body: payload["body"] = body
        if event: payload["event"] = event
        if comments: payload["comments"] = comments
        return await arequest(
            "POST", f"{GITHUB_API}/repos/{owner_repo}/pulls/{number}/reviews",
            headers=self._headers(),
            json=payload,
            expected=(200,),
            transform=lambda d: {"id": d.get("id"), "state": d.get("state"), "html_url": d.get("html_url")},
        )

    async def list_pr_reviews(self, owner_repo: str, number: int, per_page: int = 30) -> Result:
        return await arequest(
            "GET", f"{GITHUB_API}/repos/{owner_repo}/pulls/{number}/reviews",
            headers=self._headers(),
            params={"per_page": per_page},
            expected=(200,),
            transform=lambda d: {"reviews": [{"id": r.get("id"), "user": r.get("user", {}).get("login"), "state": r.get("state"), "body": r.get("body"), "submitted_at": r.get("submitted_at")} for r in d]},
        )

    async def submit_pr_review(self, owner_repo: str, number: int, review_id: int,
                               event: str, body: str = "") -> Result:
        payload: Dict[str, Any] = {"event": event}
        if body: payload["body"] = body
        return await arequest(
            "POST", f"{GITHUB_API}/repos/{owner_repo}/pulls/{number}/reviews/{review_id}/events",
            headers=self._headers(),
            json=payload,
            expected=(200,),
    ...
    async def list_pr_review_comments(self, owner_repo: str, number: int, per_page: int = 30) -> Result:
        return await arequest(
            "GET", f"{GITHUB_API}/repos/{owner_repo}/pulls/{number}/comments",
            headers=self._headers(),
            params={"per_page": per_page},
            expected=(200,),
            transform=lambda d: {"comments": [{"id": c.get("id"), "user": c.get("user", {}).get("login"), "body": c.get("body"), "path": c.get("path"), "line": c.get("line")} for c in d]},
        )

    async def create_pr_review_comment(self, owner_repo: str, number: int, body: str, commit_id: str,
                                       path: str, line: int, side: str = "RIGHT") -> Result:
        return await arequest(
            "POST", f"{GITHUB_API}/repos/{owner_repo}/pulls/{number}/comments",
            headers=self._headers(),
    ...
    async def update_issue(self, owner_repo: str, number: int, title: Optional[str] = None,
                           body: Optional[str] = None, state: Optional[str] = None,
                           labels: Optional[List[str]] = None, assignees: Optional[List[str]] = None,
                           milestone: Optional[int] = None) -> Result:
        payload: Dict[str, Any] = {}
        if title is not None: payload["title"] = title
        if body is not None: payload["body"] = body
        if state is not None: payload["state"] = state
    ...
    async def lock_issue(self, owner_repo: str, number: int, lock_reason: Optional[str] = None) -> Result:
        payload: Dict[str, Any] = {}
        if lock_reason: payload["lock_reason"] = lock_reason
        return await arequest(
            "PUT", f"{GITHUB_API}/repos/{owner_repo}/issues/{number}/lock",
            headers=self._headers(),
            json=payload,
            expected=(204,),
    ...
    async def unlock_issue(self, owner_repo: str, number: int) -> Result:
        return await arequest(
            "DELETE", f"{GITHUB_API}/repos/{owner_repo}/issues/{number}/lock",
            headers=self._headers(),
            expected=(204,),
            transform=lambda _d: {"unlocked": True, "number": number},
        )

    async def list_issue_comments(self, owner_repo: str, number: int, per_page: int = 30) -> Result:
        return await arequest(
            "GET", f"{GITHUB_API}/repos/{owner_repo}/issues/{number}/comments",
            headers=self._headers(),
            params={"per_page": per_page},
            expected=(200,),
            transform=lambda d: {"comments": [{"id": c.get("id"), "user": c.get("user", {}).get("login"), "body": c.get("body"), "created_at": c.get("created_at")} for c in d]},
        )

    async def update_issue_comment(self, owner_repo: str, comment_id: int, body: str) -> Result:
        return await arequest(
            "PATCH", f"{GITHUB_API}/repos/{owner_repo}/issues/comments/{comment_id}",
            headers=self._headers(),
            json={"body": body},
            expected=(200,),
            transform=lambda d: {"id": d.get("id"), "html_url": d.get("html_url")},
    ...
    async def delete_issue_comment(self, owner_repo: str, comment_id: int) -> Result:
        return await arequest(
            "DELETE", f"{GITHUB_API}/repos/{owner_repo}/issues/comments/{comment_id}",
            headers=self._headers(),
            expected=(204,),
            transform=lambda _d: {"deleted": True, "comment_id": comment_id},
        )

    async def list_issue_events(self, owner_repo: str, number: int, per_page: int = 30) -> Result:
        return await arequest(
            "GET", f"{GITHUB_API}/repos/{owner_repo}/issues/{number}/events",
            headers=self._headers(),
            params={"per_page": per_page},
            expected=(200,),
            transform=lambda d: {"events": [{"id": e.get("id"), "actor": e.get("actor", {}).get("login"), "event": e.get("event"), "created_at": e.get("created_at")} for e in d]},
        )

    async def remove_issue_label(self, owner_repo: str, number: int, name: str) -> Result:
        return await arequest(
            "DELETE", f"{GITHUB_API}/repos/{owner_repo}/issues/{number}/labels/{name}",
            headers=self._headers(),
            expected=(200,),
            transform=lambda d: {"labels": [l.get("name") for l in d]},
        )

    async def set_issue_labels(self, owner_repo: str, number: int, labels: List[str]) -> Result:
        return await arequest(
            "PUT", f"{GITHUB_API}/repos/{owner_repo}/issues/{number}/labels",
            headers=self._headers(),
            json={"labels": labels},
            expected=(200,),
            transform=lambda d: {"labels": [l.get("name") for l in d]},
    ...
    async def add_assignees(self, owner_repo: str, number: int, assignees: List[str]) -> Result:
        return await arequest(
            "POST", f"{GITHUB_API}/repos/{owner_repo}/issues/{number}/assignees",
            headers=self._headers(),
            json={"assignees": assignees},
            expected=(201,),
            transform=lambda d: {"number": d.get("number"), "assignees": [a.get("login") for a in d.get("assignees", [])]},
        )

    async def remove_assignees(self, owner_repo: str, number: int, assignees: List[str]) -> Result:
        return await arequest(
            "DELETE", f"{GITHUB_API}/repos/{owner_repo}/issues/{number}/assignees",
            headers=self._headers(),
            json={"assignees": assignees},
            expected=(200,),
            transform=lambda d: {"number": d.get("number"), "assignees": [a.get("login") for a in d.get("assignees", [])]},
    ...
    async def list_repo_labels(self, owner_repo: str, per_page: int = 30) -> Result:
        return await arequest(
            "GET", f"{GITHUB_API}/repos/{owner_repo}/labels",
            headers=self._headers(),
            params={"per_page": per_page},
            expected=(200,),
            transform=lambda d: {"labels": [{"name": l.get("name"), "color": l.get("color"), "description": l.get("description")} for l in d]},
        )

    async def create_label(self, owner_repo: str, name: str, color: str = "ededed",
                           description: str = "") -> Result:
        payload: Dict[str, Any] = {"name": name, "color": color}
        if description: payload["description"] = description
        return await arequest(
            "POST", f"{GITHUB_API}/repos/{owner_repo}/labels",
            headers=self._headers(),
            json=payload,
            expected=(201,),
            transform=lambda d: {"name": d.get("name"), "color": d.get("color"), "url": d.get("url")},
        )

    async def update_label(self, owner_repo: str, name: str, new_name: Optional[str] = None,
                           color: Optional[str] = None, description: Optional[str] = None) -> Result:
        payload: Dict[str, Any] = {}
        if new_name is not None: payload["new_name"] = new_name
        if color is not None: payload["color"] = color
        if description is not None: payload["description"] = description
        return await arequest(
            "PATCH", f"{GITHUB_API}/repos/{owner_repo}/labels/{name}",
            headers=self._headers(),
            json=payload,
            expected=(200,),
            transform=lambda d: {"name": d.get("name"), "color": d.get("color"), "description": d.get("description")},
        )

    async def delete_label(self, owner_repo: str, name: str) -> Result:
        return await arequest(
            "DELETE", f"{GITHUB_API}/repos/{owner_repo}/labels/{name}",
            headers=self._headers(),
            expected=(204,),
    ...
    async def list_milestones(self, owner_repo: str, state: str = "open", per_page: int = 30) -> Result:
        return await arequest(
            "GET", f"{GITHUB_API}/repos/{owner_repo}/milestones",
            headers=self._headers(),
            params={"state": state, "per_page": per_page},
            expected=(200,),
            transform=lambda d: {"milestones": [{"number": m.get("number"), "title": m.get("title"), "state": m.get("state"), "due_on": m.get("due_on"), "open_issues": m.get("open_issues"), "closed_issues": m.get("closed_issues")} for m in d]},
        )

    async def create_milestone(self, owner_repo: str, title: str, state: str = "open",
                               description: str = "", due_on: Optional[str] = None) -> Result:
        payload: Dict[str, Any] = {"title": title, "state": state}
        if description: payload["description"] = description
        if due_on: payload["due_on"] = due_on
        return await arequest(
            "POST", f"{GITHUB_API}/repos/{owner_repo}/milestones",
            headers=self._headers(),
            json=payload,
            expected=(201,),
            transform=lambda d: {"number": d.get("number"), "title": d.get("title"), "html_url": d.get("html_url")},
        )

    async def update_milestone(self, owner_repo: str, number: int, title: Optional[str] = None,
                               state: Optional[str] = None, description: Optional[str] = None,
                               due_on: Optional[str] = None) -> Result:
        payload: Dict[str, Any] = {}
        if title is not None: payload["title"] = title
        if state is not None: payload["state"] = state
        if description is not None: payload["description"] = description
        ...
        return await arequest(
            "PATCH", f"{GITHUB_API}/repos/{owner_repo}/milestones/{number}",
            headers=self._headers(),
            json=payload,
            expected=(200,),
            transform=lambda d: {"number": d.get("number"), "title": d.get("title"), "state": d.get("state")},
        )

    async def delete_milestone(self, owner_repo: str, number: int) -> Result:
        return await arequest(
            "DELETE", f"{GITHUB_API}/repos/{owner_repo}/milestones/{number}",
            headers=self._headers(),
            expected=(204,),
    ...
    async def list_releases(self, owner_repo: str, per_page: int = 30) -> Result:
        return await arequest(
            "GET", f"{GITHUB_API}/repos/{owner_repo}/releases",
            headers=self._headers(),
            params={"per_page": per_page},
            expected=(200,),
            transform=lambda d: {"releases": [{"id": r.get("id"), "tag_name": r.get("tag_name"), "name": r.get("name"), "draft": r.get("draft"), "prerelease": r.get("prerelease"), "published_at": r.get("published_at"), "html_url": r.get("html_url")} for r in d]},
        )

    async def get_release(self, owner_repo: str, release_id: Optional[int] = None,
                          tag: Optional[str] = None, latest: bool = False) -> Result:
        if latest:
    ...
    async def create_release(self, owner_repo: str, tag_name: str, name: Optional[str] = None,
                             body: str = "", draft: bool = False, prerelease: bool = False,
                             target_commitish: Optional[str] = None) -> Result:
        ...
        return await arequest(
            "POST", f"{GITHUB_API}/repos/{owner_repo}/releases",
            headers=self._headers(),
            json=payload,
            expected=(201,),
            transform=lambda d: {"id": d.get("id"), "tag_name": d.get("tag_name"), "html_url": d.get("html_url")},
        )

    async def update_release(self, owner_repo: str, release_id: int, tag_name: Optional[str] = None,
                             name: Optional[str] = None, body: Optional[str] = None,
                             draft: Optional[bool] = None, prerelease: Optional[bool] = None) -> Result:
        payload: Dict[str, Any] = {}
        if tag_name is not None: payload["tag_name"] = tag_name
        if name is not None: payload["name"] = name
        if body is not None: payload["body"] = body
        if draft is not None: payload["draft"] = draft
        if prerelease is not None: payload["prerelease"] = prerelease
        return await arequest(
            "PATCH", f"{GITHUB_API}/repos/{owner_repo}/releases/{release_id}",
            headers=self._headers(),
            json=payload,
            expected=(200,),
            transform=lambda d: {"id": d.get("id"), "tag_name": d.get("tag_name"), "html_url": d.get("html_url")},
    ...
    async def delete_release(self, owner_repo: str, release_id: int) -> Result:
        return await arequest(
            "DELETE", f"{GITHUB_API}/repos/{owner_repo}/releases/{release_id}",
            headers=self._headers(),
            expected=(204,),
    ...
    async def list_tags(self, owner_repo: str, per_page: int = 30) -> Result:
        return await arequest(
            "GET", f"{GITHUB_API}/repos/{owner_repo}/tags",
            headers=self._headers(),
            params={"per_page": per_page},
            expected=(200,),
            transform=lambda d: {"tags": [{"name": t.get("name"), "sha": t.get("commit", {}).get("sha")} for t in d]},
        )
    ...
    async def add_issue_reaction(self, owner_repo: str, number: int, content: str) -> Result:
        return await arequest(
            "POST", f"{GITHUB_API}/repos/{owner_repo}/issues/{number}/reactions",
            headers=self._headers(),
    ...
    async def add_issue_comment_reaction(self, owner_repo: str, comment_id: int, content: str) -> Result:
        return await arequest(
            "POST", f"{GITHUB_API}/repos/{owner_repo}/issues/comments/{comment_id}/reactions",
            headers=self._headers(),
            json={"content": content},
            expected=(200, 201),
            transform=lambda d: {"id": d.get("id"), "content": d.get("content")},
    ...
    async def add_pr_review_comment_reaction(self, owner_repo: str, comment_id: int, content: str) -> Result:
        return await arequest(
            "POST", f"{GITHUB_API}/repos/{owner_repo}/pulls/comments/{comment_id}/reactions",
            headers=self._headers(),
            json={"content": content},
            expected=(200, 201),
            transform=lambda d: {"id": d.get("id"), "content": d.get("content")},
    ...
    async def delete_issue_reaction(self, owner_repo: str, number: int, reaction_id: int) -> Result:
        return await arequest(
            "DELETE", f"{GITHUB_API}/repos/{owner_repo}/issues/{number}/reactions/{reaction_id}",
            headers=self._headers(),
            expected=(204,),
            transform=lambda _d: {"deleted": True, "reaction_id": reaction_id},
        )

    async def delete_issue_comment_reaction(self, owner_repo: str, comment_id: int, reaction_id: int) -> Result:
        return await arequest(
            "DELETE", f"{GITHUB_API}/repos/{owner_repo}/issues/comments/{comment_id}/reactions/{reaction_id}",
            headers=self._headers(),
            expected=(204,),
            transform=lambda _d: {"deleted": True, "reaction_id": reaction_id},
        )

    async def delete_pr_review_comment_reaction(self, owner_repo: str, comment_id: int, reaction_id: int) -> Result:
        return await arequest(
            "DELETE", f"{GITHUB_API}/repos/{owner_repo}/pulls/comments/{comment_id}/reactions/{reaction_id}",
            headers=self._headers(),
            expected=(204,),
            transform=lambda _d: {"deleted": True, "reaction_id": reaction_id},
        )
    ...
    async def search_repos(self, query: str, per_page: int = 20) -> Result:
        return await arequest(
            "GET", f"{GITHUB_API}/search/repositories",
            headers=self._headers(),
            params={"q": query, "per_page": per_page},
            timeout=30.0,
            expected=(200,),
            transform=lambda d: {
                "total_count": d.get("total_count", 0),
                "items": [{"full_name": r.get("full_name"), "html_url": r.get("html_url"), "description": r.get("description"), "stars": r.get("stargazers_count"), "language": r.get("language")} for r in d.get("items", [])],
            },
        )

    async def search_code(self, query: str, per_page: int = 20) -> Result:
        return await arequest(
            "GET", f"{GITHUB_API}/search/code",
            headers=self._headers(),
            params={"q": query, "per_page": per_page},
            timeout=30.0,
            expected=(200,),
            transform=lambda d: {
                "total_count": d.get("total_count", 0),
                "items": [{"name": i.get("name"), "path": i.get("path"), "repo": i.get("repository", {}).get("full_name"), "html_url": i.get("html_url")} for i in d.get("items", [])],
            },
        )

    async def search_users(self, query: str, per_page: int = 20) -> Result:
        return await arequest(
            "GET", f"{GITHUB_API}/search/users",
            headers=self._headers(),
            params={"q": query, "per_page": per_page},
            timeout=30.0,
            expected=(200,),
            transform=lambda d: {
                "total_count": d.get("total_count", 0),
                "items": [{"login": u.get("login"), "html_url": u.get("html_url"), "type": u.get("type")} for u in d.get("items", [])],
            },
        )

    async def search_commits(self, query: str, per_page: int = 20) -> Result:
        return await arequest(
            "GET", f"{GITHUB_API}/search/commits",
            headers=self._headers(),
            params={"q": query, "per_page": per_page},
            timeout=30.0,
            expected=(200,),
            transform=lambda d: {
                "total_count": d.get("total_count", 0),
                "items": [{"sha": c.get("sha"), "message": (c.get("commit", {}).get("message") or "").split("\n")[0], "repo": c.get("repository", {}).get("full_name"), "html_url": c.get("html_url")} for c in d.get("items", [])],
            },
        )
    ...
    async def get_user(self, username: str) -> Result:
        return await arequest(
            "GET", f"{GITHUB_API}/users/{username}",
            headers=self._headers(),
            expected=(200,),
            transform=lambda d: {"login": d.get("login"), "name": d.get("name"), "bio": d.get("bio"), "public_repos": d.get("public_repos"), "followers": d.get("followers"), "following": d.get("following"), "html_url": d.get("html_url")},
        )

    async def list_user_repos(self, username: str, per_page: int = 30, sort: str = "updated") -> Result:
        return await arequest(
            "GET", f"{GITHUB_API}/users/{username}/repos",
            headers=self._headers(),
            params={"per_page": per_page, "sort": sort},
            expected=(200,),
            transform=lambda d: {"repos": [{"full_name": r.get("full_name"), "html_url": r.get("html_url"), "description": r.get("description"), "stars": r.get("stargazers_count"), "language": r.get("language")} for r in d]},
        )

    async def follow_user(self, username: str) -> Result:
        return await arequest(
            "PUT", f"{GITHUB_API}/user/following/{username}",
            headers=self._headers(),
            expected=(204,),
    ...
    async def unfollow_user(self, username: str) -> Result:
        return await arequest(
            "DELETE", f"{GITHUB_API}/user/following/{username}",
            headers=self._headers(),
            expected=(204,),
    ...
    async def list_followers(self, per_page: int = 30) -> Result:
        return await arequest(
            "GET", f"{GITHUB_API}/user/followers",
            headers=self._headers(),
            params={"per_page": per_page},
            expected=(200,),
    ...
    async def list_following(self, per_page: int = 30) -> Result:
        return await arequest(
            "GET", f"{GITHUB_API}/user/following",
            headers=self._headers(),
            params={"per_page": per_page},
            expected=(200,),
    ...
    async def star_repo(self, owner_repo: str) -> Result:
        return await arequest(
            "PUT", f"{GITHUB_API}/user/starred/{owner_repo}",
            headers=self._headers(),
            expected=(204,),
    ...
    async def unstar_repo(self, owner_repo: str) -> Result:
        return await arequest(
            "DELETE", f"{GITHUB_API}/user/starred/{owner_repo}",
            headers=self._headers(),
            expected=(204,),
    ...
    async def list_starred(self, per_page: int = 30) -> Result:
        return await arequest(
            "GET", f"{GITHUB_API}/user/starred",
            headers=self._headers(),
            params={"per_page": per_page},
            expected=(200,),
            transform=lambda d: {"starred": [{"full_name": r.get("full_name"), "html_url": r.get("html_url")} for r in d]},
        )

    async def list_stargazers(self, owner_repo: str, per_page: int = 30) -> Result:
        return await arequest(
            "GET", f"{GITHUB_API}/repos/{owner_repo}/stargazers",
            headers=self._headers(),
            params={"per_page": per_page},
            expected=(200,),
    ...
    async def list_gists(self, per_page: int = 30) -> Result:
        return await arequest(
            "GET", f"{GITHUB_API}/gists",
            headers=self._headers(),
            params={"per_page": per_page},
            expected=(200,),
            transform=lambda d: {"gists": [{"id": g.get("id"), "description": g.get("description"), "public": g.get("public"), "html_url": g.get("html_url"), "files": list(g.get("files", {}).keys())} for g in d]},
        )

    async def get_gist(self, gist_id: str) -> Result:
        return await arequest(
            "GET", f"{GITHUB_API}/gists/{gist_id}",
            headers=self._headers(),
            expected=(200,),
        )

    async def create_gist(self, files: Dict[str, Dict[str, str]], description: str = "",
                          public: bool = True) -> Result:
        return await arequest(
            "POST", f"{GITHUB_API}/gists",
            headers=self._headers(),
    ...
    async def update_gist(self, gist_id: str, description: Optional[str] = None,
                          files: Optional[Dict[str, Dict[str, Any]]] = None) -> Result:
        payload: Dict[str, Any] = {}
        if description is not None: payload["description"] = description
        if files is not None: payload["files"] = files
        return await arequest(
            "PATCH", f"{GITHUB_API}/gists/{gist_id}",
            headers=self._headers(),
            json=payload,
            expected=(200,),
            transform=lambda d: {"id": d.get("id"), "html_url": d.get("html_url")},
    ...
    async def delete_gist(self, gist_id: str) -> Result:
        return await arequest(
            "DELETE", f"{GITHUB_API}/gists/{gist_id}",
            headers=self._headers(),
            expected=(204,),
    ...
    async def list_notifications(self, include_read: bool = False, participating: bool = False,
                                 per_page: int = 30) -> Result:
        return await arequest(
            "GET", f"{GITHUB_API}/notifications",
            headers=self._headers(),
            params={"all": str(include_read).lower(), "participating": str(participating).lower(), "per_page": per_page},
            expected=(200,),
            transform=lambda d: {"notifications": [{"id": n.get("id"), "reason": n.get("reason"), "unread": n.get("unread"), "repo": n.get("repository", {}).get("full_name"), "subject": n.get("subject", {}).get("title"), "type": n.get("subject", {}).get("type")} for n in d]},
        )

    async def mark_all_notifications_read(self, last_read_at: Optional[str] = None) -> Result:
        payload: Dict[str, Any] = {}
        if last_read_at: payload["last_read_at"] = last_read_at
        return await arequest(
            "PUT", f"{GITHUB_API}/notifications",
            headers=self._headers(),
            json=payload,
            expected=(202, 205),
    ...
    async def list_workflows(self, owner_repo: str, per_page: int = 30) -> Result:
        return await arequest(
            "GET", f"{GITHUB_API}/repos/{owner_repo}/actions/workflows",
            headers=self._headers(),
            params={"per_page": per_page},
            expected=(200,),
            transform=lambda d: {"workflows": [{"id": w.get("id"), "name": w.get("name"), "path": w.get("path"), "state": w.get("state")} for w in d.get("workflows", [])]},
        )

    async def list_workflow_runs(self, owner_repo: str, workflow_id: Optional[str] = None,
                                 branch: Optional[str] = None, status: Optional[str] = None,
                                 per_page: int = 30) -> Result:
        params: Dict[str, Any] = {"per_page": per_page}
        if branch: params["branch"] = branch
        if status: params["status"] = status
        url = (f"{GITHUB_API}/repos/{owner_repo}/actions/workflows/{workflow_id}/runs"
               if workflow_id else f"{GITHUB_API}/repos/{owner_repo}/actions/runs")
        return await arequest(
            "GET", url,
            headers=self._headers(),
            params=params,
            expected=(200,),
            transform=lambda d: {"workflow_runs": [{"id": r.get("id"), "name": r.get("name"), "status": r.get("status"), "conclusion": r.get("conclusion"), "branch": r.get("head_branch"), "html_url": r.get("html_url"), "created_at": r.get("created_at")} for r in d.get("workflow_runs", [])]},
        )

    async def get_workflow_run(self, owner_repo: str, run_id: int) -> Result:
        return await arequest(
            "GET", f"{GITHUB_API}/repos/{owner_repo}/actions/runs/{run_id}",
            headers=self._headers(),
            expected=(200,),
            transform=lambda d: {"id": d.get("id"), "name": d.get("name"), "status": d.get("status"), "conclusion": d.get("conclusion"), "branch": d.get("head_branch"), "html_url": d.get("html_url")},
        )

    async def trigger_workflow(self, owner_repo: str, workflow_id: str, ref: str,
                               inputs: Optional[Dict[str, Any]] = None) -> Result:
        payload: Dict[str, Any] = {"ref": ref}
        if inputs: payload["inputs"] = inputs
        return await arequest(
            "POST", f"{GITHUB_API}/repos/{owner_repo}/actions/workflows/{workflow_id}/dispatches",
            headers=self._headers(),
            json=payload,
            expected=(204,),
            transform=lambda _d: {"triggered": True, "workflow_id": workflow_id, "ref": ref},
        )

    async def cancel_workflow_run(self, owner_repo: str, run_id: int) -> Result:
        return await arequest(
            "POST", f"{GITHUB_API}/repos/{owner_repo}/actions/runs/{run_id}/cancel",
            headers=self._headers(),
            expected=(202,),
    ...
    async def rerun_workflow_run(self, owner_repo: str, run_id: int) -> Result:
        return await arequest(
            "POST", f"{GITHUB_API}/repos/{owner_repo}/actions/runs/{run_id}/rerun",
            headers=self._headers(),
            expected=(201,),
    ...
    async def get_workflow_run_logs_url(self, owner_repo: str, run_id: int) -> Result:
        ...
        try:
            ...
            if r.status_code == 302:
                return {"ok": True, "result": {"logs_url": r.headers.get("location", "")}}
