...
@register_handler(STRIPE.name)
class StripeHandler(IntegrationHandler):
    spec = STRIPE
    display_name = "Stripe"
    ...
    config_fields = [
        {
            ...
            "type": "text",
            "placeholder": "usd",
            "help": "ISO 4217 lowercase code used when create_* actions omit 'currency'. "
            "Stripe stores amounts as integers in the smallest currency unit (cents for USD).",
        },
        {
            "key": "default_expand",
            ...
            "help": "Comma-separated field paths auto-added to expand[] on every retrieve/list. "
            "Useful when downstream actions chain off nested objects. Empty by default.",
        },
        {
            "key": "require_idempotency_key",
            "label": "Auto-attach Idempotency-Key on mutations",
            "type": "checkbox",
            "help": "When enabled, the client sends a UUID Idempotency-Key on every "
            "POST/DELETE so that an agent retry doesn't double-create resources.",
        },
    ]
    ...
    async def login(self, args: List[str]) -> Tuple[bool, str]:
        if not args:
            return False, (
                "Usage: /stripe login <api_key>\n"
                "Get a key at: https://dashboard.stripe.com/apikeys\n"
                "Restricted keys (rk_…) are strongly preferred over secret keys (sk_…)."
            )
        token = args[0].strip()
        if token.startswith("pk_"):
            return False, (
                "That's a publishable key (pk_…). Publishable keys are for client-side "
                "code and won't authenticate server-side requests. Paste a secret (sk_…) "
                "or restricted (rk_…) key instead."
            )
        if not (token.startswith("sk_") or token.startswith("rk_")):
            return False, (
                "Invalid Stripe key. Expected sk_live_…, sk_test_…, rk_live_…, or rk_test_…."
            )

        # Validate by fetching the merchant's own account. /v1/account works for
        # secret keys; restricted keys may or may not have it scoped — if it 401s
        # we fall back to /v1/balance which every authenticated key can reach.
        ...
        if "error" not in acct:
            ...
            if "error" in balance:
                return False, f"Stripe auth failed: {balance['error']}"
            # /balance succeeded; the restricted key lacks the 'account read' scope.
            # That's fine — store without account_id.
        ...
        return True, f"Stripe connected: {label} ({mode}, {kind_label})"

    async def logout(self, args: List[str]) -> Tuple[bool, str]:
        if not has_credential(self.spec.cred_file):
            return False, "No Stripe credentials found."
        try:
        ...
        return True, "Removed Stripe credential."

    async def status(self) -> Tuple[bool, str]:
        if not has_credential(self.spec.cred_file):
            return True, "Stripe: Not connected"
        cred = load_credential(self.spec.cred_file, StripeCredential)
        if not cred:
            return True, "Stripe: Not connected"
        label = cred.business_name or cred.account_id or "unknown account"
        mode = "live mode" if cred.livemode else "TEST MODE"
        kind = "restricted key" if cred.key_kind == "restricted" else "secret key"
        return True, f"Stripe: Connected\n  - {label} ({mode}, {kind})"


# ------------------------------------------------------------------
# Form-encoding helpers — Stripe's API takes application/x-www-form-urlencoded
# with bracket notation for nested params:
#
#   {"metadata": {"order": "6735"}}        → metadata[order]=6735
#   {"expand": ["customer"]}               → expand[]=customer
#   {"items": [{"price": "p_x"}]}          → items[0][price]=p_x
#   {"automatic_payment_methods": {"enabled": True}}
#                                          → automatic_payment_methods[enabled]=true
#
# httpx's data= argument form-encodes a FLAT dict; we have to flatten ourselves.
# ------------------------------------------------------------------
