...
class OutlookClient(BasePlatformClient):
    ...
    def reply_to_message(self, message_id: str, comment: str,
                         to_recipients: Optional[List[str]] = None) -> Result:
        ...
        return http_request(
            "POST", f"{GRAPH_API_BASE}/me/messages/{message_id}/reply",
            headers=self._headers(), json=payload, expected=(202,),
            transform=lambda _d: {"replied": True, "message_id": message_id},
        )

    def reply_all_to_message(self, message_id: str, comment: str) -> Result:
        return http_request(
            "POST", f"{GRAPH_API_BASE}/me/messages/{message_id}/replyAll",
            headers=self._headers(), json={"comment": comment}, expected=(202,),
            transform=lambda _d: {"replied_all": True, "message_id": message_id},
        )

    def forward_message(self, message_id: str, to_recipients: List[str],
                        comment: str = "") -> Result:
        return http_request(
            "POST", f"{GRAPH_API_BASE}/me/messages/{message_id}/forward",
            headers=self._headers(),
            json={"comment": comment, "toRecipients": self._recipients(to_recipients)},
            expected=(202,),
            transform=lambda _d: {"forwarded": True, "message_id": message_id, "to": to_recipients},
        )
    ...
    def create_reply_draft(self, message_id: str, comment: str = "") -> Result:
        ...
        return http_request(
            "POST", f"{GRAPH_API_BASE}/me/messages/{message_id}/createReply",
            headers=self._headers(), json=payload, expected=(201,),
            transform=lambda d: {"draft_id": d.get("id"), "conversationId": d.get("conversationId")},
        )

    def create_forward_draft(self, message_id: str, to_recipients: List[str],
                             comment: str = "") -> Result:
        payload: Dict[str, Any] = {
            "comment": comment,
        ...
        return http_request(
            "POST", f"{GRAPH_API_BASE}/me/messages/{message_id}/createForward",
            headers=self._headers(), json=payload, expected=(201,),
            transform=lambda d: {"draft_id": d.get("id"), "conversationId": d.get("conversationId")},
        )

    def create_draft(self, subject: str, body: str, to: Optional[List[str]] = None,
        ...
        if to: message["toRecipients"] = self._recipients(to)
        if cc: message["ccRecipients"] = self._recipients(cc)
        if bcc: message["bccRecipients"] = self._recipients(bcc)
        return http_request(
            "POST", f"{GRAPH_API_BASE}/me/messages",
            headers=self._headers(), json=message, expected=(201,),
            transform=lambda d: {"draft_id": d.get("id"), "subject": d.get("subject"),
                                 "conversationId": d.get("conversationId")},
        )

    def update_draft(self, message_id: str, subject: Optional[str] = None,
                     body: Optional[str] = None, html: bool = False,
                     to: Optional[List[str]] = None, cc: Optional[List[str]] = None,
                     bcc: Optional[List[str]] = None) -> Result:
        payload: Dict[str, Any] = {}
        if subject is not None: payload["subject"] = subject
        if body is not None:
            payload["body"] = {"contentType": "HTML" if html else "Text", "content": body}
        if to is not None: payload["toRecipients"] = self._recipients(to)
        if cc is not None: payload["ccRecipients"] = self._recipients(cc)
        if bcc is not None: payload["bccRecipients"] = self._recipients(bcc)
        return http_request(
    ...
    def send_draft(self, message_id: str) -> Result:
        ...
        return http_request(
            "POST", f"{GRAPH_API_BASE}/me/messages/{message_id}/send",
            headers=self._headers(), expected=(202,),
            transform=lambda _d: {"sent": True, "message_id": message_id},
        )

    def delete_message(self, message_id: str) -> Result:
        return http_request(
            "DELETE", f"{GRAPH_API_BASE}/me/messages/{message_id}",
    ...
    def move_message(self, message_id: str, destination_folder_id: str) -> Result:
        ...
        return http_request(
            "POST", f"{GRAPH_API_BASE}/me/messages/{message_id}/move",
            headers=self._headers(), json={"destinationId": destination_folder_id},
            expected=(201,),
            transform=lambda d: {"moved": True, "new_id": d.get("id"), "parent_folder_id": d.get("parentFolderId")},
        )

    def copy_message(self, message_id: str, destination_folder_id: str) -> Result:
        return http_request(
            "POST", f"{GRAPH_API_BASE}/me/messages/{message_id}/copy",
            headers=self._headers(), json={"destinationId": destination_folder_id},
            expected=(201,),
    ...
    def mark_as_unread(self, message_id: str) -> Result:
        return http_request(
            "PATCH", f"{GRAPH_API_BASE}/me/messages/{message_id}",
            headers=self._headers(), json={"isRead": False}, expected=(200,),
    ...
    def flag_message(self, message_id: str, flag_status: str = "flagged") -> Result:
        ...
        return http_request(
            "PATCH", f"{GRAPH_API_BASE}/me/messages/{message_id}",
            headers=self._headers(),
            json={"flag": {"flagStatus": flag_status}}, expected=(200,),
    ...
    def set_message_categories(self, message_id: str, categories: List[str]) -> Result:
        return http_request(
            "PATCH", f"{GRAPH_API_BASE}/me/messages/{message_id}",
            headers=self._headers(),
            json={"categories": categories}, expected=(200,),
            transform=lambda _d: {"categories": categories, "message_id": message_id},
        )

    def search_messages(self, query: str, top: int = 25,
                        folder: Optional[str] = None) -> Result:
        ...
        url = (f"{GRAPH_API_BASE}/me/mailFolders/{folder}/messages"
               if folder else f"{GRAPH_API_BASE}/me/messages")
        return http_request(
            "GET", url,
            headers=self._auth_header(),
            params={
    ...
    def list_attachments(self, message_id: str) -> Result:
        return http_request(
            "GET", f"{GRAPH_API_BASE}/me/messages/{message_id}/attachments",
            headers=self._auth_header(),
    ...
    def get_attachment(self, message_id: str, attachment_id: str) -> Result:
        return http_request(
            "GET", f"{GRAPH_API_BASE}/me/messages/{message_id}/attachments/{attachment_id}",
    ...
    def download_attachment(self, message_id: str, attachment_id: str,
        ...
        if not content_b64:
            return {"error": "Attachment has no contentBytes (may be itemAttachment or referenceAttachment, not fileAttachment)"}
        try:
            ...
            return {"ok": True, "result": {"saved_to": save_to, "size": os.path.getsize(save_to)}}
        except Exception as e:
            return {"error": str(e)}

    def add_attachment(self, message_id: str, file_path: str,
                       content_type: Optional[str] = None) -> Result:
        ...
        return http_request(
            "POST", f"{GRAPH_API_BASE}/me/messages/{message_id}/attachments",
            headers=self._headers(), json=payload, expected=(201,),
            transform=lambda d: {"id": d.get("id"), "name": d.get("name"), "size": d.get("size")},
        )

    def delete_attachment(self, message_id: str, attachment_id: str) -> Result:
        return http_request(
            "DELETE", f"{GRAPH_API_BASE}/me/messages/{message_id}/attachments/{attachment_id}",
            headers=self._auth_header(), expected=(204,),
    ...
    def get_folder(self, folder_id: str) -> Result:
        return http_request(
            "GET", f"{GRAPH_API_BASE}/me/mailFolders/{folder_id}",
            headers=self._auth_header(), expected=(200,),
            transform=lambda d: {
                "id": d.get("id"), "name": d.get("displayName"),
                "parentFolderId": d.get("parentFolderId"),
                "total": d.get("totalItemCount"), "unread": d.get("unreadItemCount"),
            },
        )

    def create_folder(self, display_name: str,
                      parent_folder_id: str = "msgfolderroot") -> Result:
        return http_request(
            "POST", f"{GRAPH_API_BASE}/me/mailFolders/{parent_folder_id}/childFolders",
            headers=self._headers(),
            json={"displayName": display_name}, expected=(201,),
            transform=lambda d: {"id": d.get("id"), "name": d.get("displayName")},
        )

    def update_folder(self, folder_id: str, display_name: str) -> Result:
        return http_request(
            "PATCH", f"{GRAPH_API_BASE}/me/mailFolders/{folder_id}",
            headers=self._headers(),
            json={"displayName": display_name}, expected=(200,),
            transform=lambda d: {"id": d.get("id"), "name": d.get("displayName")},
        )

    def delete_folder(self, folder_id: str) -> Result:
        return http_request(
            "DELETE", f"{GRAPH_API_BASE}/me/mailFolders/{folder_id}",
            headers=self._auth_header(), expected=(204,),
    ...
    def list_child_folders(self, folder_id: str = "msgfolderroot") -> Result:
        return http_request(
            "GET", f"{GRAPH_API_BASE}/me/mailFolders/{folder_id}/childFolders",
            headers=self._auth_header(),
            params={"$select": "id,displayName,totalItemCount,unreadItemCount"},
            expected=(200,),
    ...
    def list_folder_messages(self, folder_id: str, n: int = 25,
                             unread_only: bool = False) -> Result:
        params: Dict[str, Any] = {
        ...
        if unread_only:
            params["$filter"] = "isRead eq false"
        return http_request(
            "GET", f"{GRAPH_API_BASE}/me/mailFolders/{folder_id}/messages",
            headers=self._auth_header(), params=params, expected=(200,),
            transform=lambda d: {"messages": [
                {
                    "id": m.get("id"),
                    "from": (m.get("from") or {}).get("emailAddress", {}).get("address", ""),
                    "subject": m.get("subject", ""),
    ...
    def get_mailbox_settings(self) -> Result:
        return http_request(
            "GET", f"{GRAPH_API_BASE}/me/mailboxSettings",
            headers=self._auth_header(), expected=(200,),
        )

    def update_mailbox_settings(self, settings: Dict[str, Any]) -> Result:
        return http_request(
            "PATCH", f"{GRAPH_API_BASE}/me/mailboxSettings",
            headers=self._headers(), json=settings, expected=(200,),
        )

    def get_automatic_replies(self) -> Result:
        return http_request(
            "GET", f"{GRAPH_API_BASE}/me/mailboxSettings/automaticRepliesSetting",
            headers=self._auth_header(), expected=(200,),
        )

    def update_automatic_replies(self, status: str,
                                 internal_reply: Optional[str] = None,
                                 external_reply: Optional[str] = None,
                                 external_audience: str = "all",
                                 scheduled_start: Optional[str] = None,
                                 scheduled_end: Optional[str] = None) -> Result:
        ...
        if internal_reply is not None: ars["internalReplyMessage"] = internal_reply
        if external_reply is not None: ars["externalReplyMessage"] = external_reply
        if scheduled_start and scheduled_end:
            ars["scheduledStartDateTime"] = {"dateTime": scheduled_start, "timeZone": "UTC"}
            ars["scheduledEndDateTime"] = {"dateTime": scheduled_end, "timeZone": "UTC"}
        return http_request(
            "PATCH", f"{GRAPH_API_BASE}/me/mailboxSettings",
            headers=self._headers(), json=payload, expected=(200,),
            transform=lambda d: {"status": d.get("automaticRepliesSetting", {}).get("status")},
        )
    ...
    def list_inbox_rules(self) -> Result:
        return http_request(
            "GET", f"{GRAPH_API_BASE}/me/mailFolders/inbox/messageRules",
            headers=self._auth_header(), expected=(200,),
            transform=lambda d: {"rules": [
                {"id": r.get("id"), "name": r.get("displayName"),
                 "sequence": r.get("sequence"), "enabled": r.get("isEnabled")}
                for r in d.get("value", [])
            ]},
        )

    def create_inbox_rule(self, display_name: str, conditions: Dict[str, Any],
                          actions: Dict[str, Any], sequence: int = 1,
                          is_enabled: bool = True) -> Result:
        payload = {
        ...
        return http_request(
            "POST", f"{GRAPH_API_BASE}/me/mailFolders/inbox/messageRules",
            headers=self._headers(), json=payload, expected=(201,),
            transform=lambda d: {"id": d.get("id"), "name": d.get("displayName")},
        )
    ...
    def delete_inbox_rule(self, rule_id: str) -> Result:
        return http_request(
            "DELETE", f"{GRAPH_API_BASE}/me/mailFolders/inbox/messageRules/{rule_id}",
            headers=self._auth_header(), expected=(204,),
    ...
    def list_categories(self) -> Result:
        return http_request(
            "GET", f"{GRAPH_API_BASE}/me/outlook/masterCategories",
            headers=self._auth_header(), expected=(200,),
            transform=lambda d: {"categories": [
                {"id": c.get("id"), "displayName": c.get("displayName"), "color": c.get("color")}
    ...
    def create_category(self, display_name: str, color: str = "preset0") -> Result:
        ...
        return http_request(
            "POST", f"{GRAPH_API_BASE}/me/outlook/masterCategories",
            headers=self._headers(),
            json={"displayName": display_name, "color": color}, expected=(201,),
            transform=lambda d: {"id": d.get("id"), "displayName": d.get("displayName"), "color": d.get("color")},
        )

    def delete_category(self, category_id: str) -> Result:
        return http_request(
            "DELETE", f"{GRAPH_API_BASE}/me/outlook/masterCategories/{category_id}",
            headers=self._auth_header(), expected=(204,),
