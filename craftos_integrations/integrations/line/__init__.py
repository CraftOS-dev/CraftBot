...
class LineClient(BasePlatformClient):
    ...
    def push_messages(self, to: str, messages: List[Dict[str, Any]],
                      notification_disabled: Optional[bool] = None) -> Result:
        ...
        """
        cfg = self._config()
        nd = cfg.notification_disabled if notification_disabled is None else notification_disabled
    ...
    def reply_messages(self, reply_token: str, messages: List[Dict[str, Any]],
                       notification_disabled: Optional[bool] = None) -> Result:
    ...
    def multicast_messages(self, to: List[str], messages: List[Dict[str, Any]],
                           notification_disabled: Optional[bool] = None) -> Result:
        cfg = self._config()
        nd = cfg.notification_disabled if notification_disabled is None else notification_disabled
        payload: Dict[str, Any] = {"to": to, "messages": messages}
    ...
    def broadcast_messages(self, messages: List[Dict[str, Any]],
                           notification_disabled: Optional[bool] = None) -> Result:
        cfg = self._config()
        nd = cfg.notification_disabled if notification_disabled is None else notification_disabled
    ...
    def push_image(self, to: str, original_content_url: str,
                   preview_image_url: Optional[str] = None) -> Result:
        msg: Dict[str, Any] = {"type": "image",
                                "originalContentUrl": original_content_url,
                                "previewImageUrl": preview_image_url or original_content_url}
        return self.push_messages(to, [msg])

    def push_video(self, to: str, original_content_url: str,
                   preview_image_url: str) -> Result:
        msg: Dict[str, Any] = {"type": "video",
                                "originalContentUrl": original_content_url,
                                "previewImageUrl": preview_image_url}
        return self.push_messages(to, [msg])

    def push_audio(self, to: str, original_content_url: str,
                   duration_ms: int) -> Result:
        msg: Dict[str, Any] = {"type": "audio",
                                "originalContentUrl": original_content_url,
                                "duration": duration_ms}
        return self.push_messages(to, [msg])

    def push_location(self, to: str, title: str, address: str,
                      latitude: float, longitude: float) -> Result:
        msg: Dict[str, Any] = {"type": "location", "title": title,
                                "address": address,
                                "latitude": latitude, "longitude": longitude}
        return self.push_messages(to, [msg])

    def push_sticker(self, to: str, package_id: str, sticker_id: str) -> Result:
        msg: Dict[str, Any] = {"type": "sticker",
    ...
    def push_template(self, to: str, alt_text: str, template: Dict[str, Any]) -> Result:
        ...
        msg: Dict[str, Any] = {"type": "template", "altText": alt_text, "template": template}
        return self.push_messages(to, [msg])

    def push_imagemap(self, to: str, base_url: str, alt_text: str,
                      base_width: int, base_height: int,
                      actions: List[Dict[str, Any]]) -> Result:
        msg: Dict[str, Any] = {
            "type": "imagemap", "baseUrl": base_url, "altText": alt_text,
    ...
    def get_message_content(self, message_id: str, dest_path: str) -> Result:
        ...
        try:
            with httpx.stream(
                "GET", f"{LINE_DATA_API_BASE}/message/{message_id}/content",
                headers={"Authorization": f"Bearer {self._load().channel_access_token}"},
                timeout=120.0,
            ) as resp:
                ...
                return {"ok": True, "result": {
                    "path": dest_path, "bytes_written": bytes_written,
    ...
    def get_group_summary(self, group_id: str) -> Result:
        return http_request(
            "GET", f"{LINE_API_BASE}/group/{group_id}/summary",
            headers={"Authorization": f"Bearer {self._load().channel_access_token}"},
            expected=(200,),
        )
    ...
    def get_group_member_count(self, group_id: str) -> Result:
        return http_request(
            "GET", f"{LINE_API_BASE}/group/{group_id}/members/count",
            headers={"Authorization": f"Bearer {self._load().channel_access_token}"},
            expected=(200,),
        )

    def list_group_member_user_ids(self, group_id: str,
                                   start: Optional[str] = None) -> Result:
        params: Dict[str, Any] = {}
        if start:
            params["start"] = start
        return http_request(
            "GET", f"{LINE_API_BASE}/group/{group_id}/members/ids",
            headers={"Authorization": f"Bearer {self._load().channel_access_token}"},
    ...
    def get_group_member_profile(self, group_id: str, user_id: str) -> Result:
        return http_request(
            "GET", f"{LINE_API_BASE}/group/{group_id}/member/{user_id}",
            headers={"Authorization": f"Bearer {self._load().channel_access_token}"},
            expected=(200,),
        )
    ...
    def leave_group(self, group_id: str) -> Result:
        return http_request(
            "POST", f"{LINE_API_BASE}/group/{group_id}/leave",
    ...
    def get_room_member_count(self, room_id: str) -> Result:
        return http_request(
            "GET", f"{LINE_API_BASE}/room/{room_id}/members/count",
            headers={"Authorization": f"Bearer {self._load().channel_access_token}"},
            expected=(200,),
        )

    def list_room_member_user_ids(self, room_id: str,
                                  start: Optional[str] = None) -> Result:
        params: Dict[str, Any] = {}
        if start:
            params["start"] = start
        return http_request(
            "GET", f"{LINE_API_BASE}/room/{room_id}/members/ids",
            headers={"Authorization": f"Bearer {self._load().channel_access_token}"},
            params=params, expected=(200,),
        )
    ...
    def get_room_member_profile(self, room_id: str, user_id: str) -> Result:
        return http_request(
            "GET", f"{LINE_API_BASE}/room/{room_id}/member/{user_id}",
            headers={"Authorization": f"Bearer {self._load().channel_access_token}"},
            expected=(200,),
        )
    ...
    def leave_room(self, room_id: str) -> Result:
        return http_request(
            "POST", f"{LINE_API_BASE}/room/{room_id}/leave",
            headers=self._headers(), expected=(200,),
    ...
    def create_rich_menu(self, rich_menu: Dict[str, Any]) -> Result:
        ...
        return http_request(
            "POST", f"{LINE_API_BASE}/richmenu",
            headers=self._headers(), json=rich_menu, expected=(200,),
        )

    def get_rich_menu(self, rich_menu_id: str) -> Result:
        return http_request(
            "GET", f"{LINE_API_BASE}/richmenu/{rich_menu_id}",
            headers={"Authorization": f"Bearer {self._load().channel_access_token}"},
            expected=(200,),
        )
    ...
    def list_rich_menus(self) -> Result:
        return http_request(
            "GET", f"{LINE_API_BASE}/richmenu/list",
            headers={"Authorization": f"Bearer {self._load().channel_access_token}"},
            expected=(200,),
        )
    ...
    def delete_rich_menu(self, rich_menu_id: str) -> Result:
        return http_request(
            "DELETE", f"{LINE_API_BASE}/richmenu/{rich_menu_id}",
            headers={"Authorization": f"Bearer {self._load().channel_access_token}"},
            expected=(200,),
    ...
    def upload_rich_menu_image(self, rich_menu_id: str, file_path: str) -> Result:
        ...
        if mime not in ("image/png", "image/jpeg"):
            return {"error": f"Unsupported image type {mime}; rich menu images must be PNG or JPEG"}
        try:
            ...
            if r.status_code != 200:
                return {"error": f"Upload failed: HTTP {r.status_code}", "details": r.text[:500]}
            return {"ok": True, "result": {"rich_menu_id": rich_menu_id, "size": len(data)}}
        except (httpx.HTTPError, OSError) as e:
            return {"error": str(e)}

    def set_default_rich_menu(self, rich_menu_id: str) -> Result:
        return http_request(
            "POST", f"{LINE_API_BASE}/user/all/richmenu/{rich_menu_id}",
            headers={"Authorization": f"Bearer {self._load().channel_access_token}"},
            expected=(200,),
    ...
    def get_default_rich_menu(self) -> Result:
        return http_request(
            "GET", f"{LINE_API_BASE}/user/all/richmenu",
            headers={"Authorization": f"Bearer {self._load().channel_access_token}"},
            expected=(200,),
        )
    ...
    def cancel_default_rich_menu(self) -> Result:
        return http_request(
            "DELETE", f"{LINE_API_BASE}/user/all/richmenu",
            headers={"Authorization": f"Bearer {self._load().channel_access_token}"},
            expected=(200,),
    ...
    def link_rich_menu_to_user(self, user_id: str, rich_menu_id: str) -> Result:
        return http_request(
            "POST", f"{LINE_API_BASE}/user/{user_id}/richmenu/{rich_menu_id}",
            headers={"Authorization": f"Bearer {self._load().channel_access_token}"},
            expected=(200,),
            transform=lambda _d: {"linked": True, "user_id": user_id, "rich_menu_id": rich_menu_id},
        )

    def unlink_rich_menu_from_user(self, user_id: str) -> Result:
        return http_request(
            "DELETE", f"{LINE_API_BASE}/user/{user_id}/richmenu",
            headers={"Authorization": f"Bearer {self._load().channel_access_token}"},
            expected=(200,),
    ...
    def get_user_rich_menu(self, user_id: str) -> Result:
        return http_request(
            "GET", f"{LINE_API_BASE}/user/{user_id}/richmenu",
            headers={"Authorization": f"Bearer {self._load().channel_access_token}"},
            expected=(200,),
        )

    def bulk_link_rich_menu(self, rich_menu_id: str,
                            user_ids: List[str]) -> Result:
        """Link 1+ users (max 500) to a rich menu in one call."""
        return http_request(
            "POST", f"{LINE_API_BASE}/richmenu/bulk/link",
            headers=self._headers(),
    ...
    def bulk_unlink_rich_menu(self, user_ids: List[str]) -> Result:
        return http_request(
            "POST", f"{LINE_API_BASE}/richmenu/bulk/unlink",
            headers=self._headers(),
    ...
    def send_narrowcast(self, messages: List[Dict[str, Any]],
        ...
        nd = cfg.notification_disabled if notification_disabled is None else notification_disabled
        payload: Dict[str, Any] = {"messages": messages}
        if recipient is not None: payload["recipient"] = recipient
        if demographic is not None: payload["filter"] = {"demographic": demographic}
        if limit is not None: payload["limit"] = limit
        if nd: payload["notificationDisabled"] = True
        return http_request(
            "POST", f"{LINE_API_BASE}/message/narrowcast",
            headers=self._headers(), json=payload, expected=(202,),
            transform=lambda d: {"request_id": d.get("requestId") if d else None,
                                 "queued": True},
        )

    def get_narrowcast_progress(self, request_id: str) -> Result:
        return http_request(
            "GET", f"{LINE_API_BASE}/message/progress/narrowcast",
            headers={"Authorization": f"Bearer {self._load().channel_access_token}"},
            params={"requestId": request_id}, expected=(200,),
        )

    def create_user_id_audience(self, description: str,
                                audiences: Optional[List[Dict[str, str]]] = None,
                                is_ifa_audience: bool = False) -> Result:
    ...
    def update_audience_description(self, audience_group_id: int,
                                    description: str) -> Result:
        return http_request(
            "PUT", f"{LINE_API_BASE}/audienceGroup/{audience_group_id}/updateDescription",
            headers=self._headers(),
            json={"description": description}, expected=(200,),
            transform=lambda _d: {"updated": True, "audience_group_id": audience_group_id},
        )

    def delete_audience(self, audience_group_id: int) -> Result:
        return http_request(
            "DELETE", f"{LINE_API_BASE}/audienceGroup/{audience_group_id}",
            headers={"Authorization": f"Bearer {self._load().channel_access_token}"},
            expected=(200,),
            transform=lambda _d: {"deleted": True, "audience_group_id": audience_group_id},
        )

    def get_audience(self, audience_group_id: int) -> Result:
        return http_request(
            "GET", f"{LINE_API_BASE}/audienceGroup/{audience_group_id}",
            headers={"Authorization": f"Bearer {self._load().channel_access_token}"},
            expected=(200,),
        )

    def list_audiences(self, page: int = 1, size: int = 20,
                       description: Optional[str] = None,
                       status: Optional[str] = None) -> Result:
        params: Dict[str, Any] = {"page": page, "size": min(size, 40)}
        if description: params["description"] = description
        if status: params["status"] = status
        return http_request(
            "GET", f"{LINE_API_BASE}/audienceGroup/list",
            headers={"Authorization": f"Bearer {self._load().channel_access_token}"},
            params=params, expected=(200,),
        )
    ...
    def get_number_of_followers(self, date: str) -> Result:
        """date: YYYYMMDD."""
        return http_request(
            "GET", f"{LINE_API_BASE}/insight/followers",
            headers={"Authorization": f"Bearer {self._load().channel_access_token}"},
            params={"date": date}, expected=(200,),
        )

    def get_friend_demographics(self) -> Result:
        return http_request(
            "GET", f"{LINE_API_BASE}/insight/demographic",
            headers={"Authorization": f"Bearer {self._load().channel_access_token}"},
            expected=(200,),
        )
    ...
    def get_message_delivery_stats(self, date: str) -> Result:
        ...
        return http_request(
            "GET", f"{LINE_API_BASE}/insight/message/delivery",
            headers={"Authorization": f"Bearer {self._load().channel_access_token}"},
            params={"date": date}, expected=(200,),
        )
    ...
    def get_message_event_stats(self, request_id: str) -> Result:
        ...
        return http_request(
            "GET", f"{LINE_API_BASE}/insight/message/event",
            headers={"Authorization": f"Bearer {self._load().channel_access_token}"},
            params={"requestId": request_id}, expected=(200,),
        )
    ...
    def get_user_interaction_stats(self, request_id: str) -> Result:
        return http_request(
            "GET", f"{LINE_API_BASE}/insight/message/event/aggregation",
            headers={"Authorization": f"Bearer {self._load().channel_access_token}"},
            params={"customAggregationUnit": request_id}, expected=(200,),
        )
    ...
    def set_webhook_endpoint(self, endpoint: str) -> Result:
        return http_request(
            "PUT", f"{LINE_API_BASE}/channel/webhook/endpoint",
            headers=self._headers(),
            json={"endpoint": endpoint}, expected=(200,),
            transform=lambda _d: {"endpoint": endpoint, "updated": True},
        )

    def get_webhook_endpoint(self) -> Result:
        return http_request(
            "GET", f"{LINE_API_BASE}/channel/webhook/endpoint",
            headers={"Authorization": f"Bearer {self._load().channel_access_token}"},
            expected=(200,),
        )
    ...
    def test_webhook_endpoint(self, endpoint: Optional[str] = None) -> Result:
        ...
        return http_request(
            "POST", f"{LINE_API_BASE}/channel/webhook/test",
            headers=self._headers(), json=payload, expected=(200,),
        )

    def issue_channel_access_token(self, channel_id: str,
                                   channel_secret: str) -> Result:
        """Issue a short-lived access token (v2.1) — useful for rotating credentials."""
        return http_request(
            "POST", f"{LINE_OAUTH_API_BASE}/token",
    ...
    def revoke_channel_access_token(self, access_token: str) -> Result:
        return http_request(
            "POST", f"{LINE_OAUTH_API_BASE}/revoke",
    ...
    def verify_access_token(self, access_token: str) -> Result:
        return http_request(
            "GET", f"{LINE_OAUTH_API_BASE}/verify",
            params={"access_token": access_token}, expected=(200,),
        )
