...
export function Drawer({
  ...
  return createPortal(
    ...
    >
      ...
      >
        <div className="flex items-center justify-between gap-3 border-b border-[var(--agent-app-border)] px-4 py-3">
          <h2 className="truncate text-base font-semibold">{title}</h2>
          <button
            type="button"
            onClick={onClose}
            aria-label="Close panel"
            className="inline-flex shrink-0 rounded p-1 text-[var(--agent-app-muted)] transition-colors hover:bg-[var(--agent-app-border)]/40 hover:text-[var(--agent-app-text)]"
          >
            <XIcon size={18} />
          </button>
        </div>
        ...
        {footer !== undefined && footer !== null && (
          <div className="flex justify-end gap-2 border-t border-[var(--agent-app-border)] p-4">
            {footer}
          </div>
        )}
      </div>
